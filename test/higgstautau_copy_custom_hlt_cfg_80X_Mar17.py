...
#opts.register('file', 'file:root://xrootd.grid.hep.ph.ic.ac.uk//store/user/adow/VBFPFglobalplusTauSignalSamples/VBFHToTauTau_M125_13TeV_powheg_pythia8/VBFPFglobalplusTauSignalSamples/170601_142232/0000/outputFULL_101.root', parser.VarParsing.multiplicity.singleton,
opts.register('file', 'root://xrootd.grid.hep.ph.ic.ac.uk//store/user/adow/92X_LooseIsoPFTau20_83XSignalSamples/VBFHToTauTau_M125_13TeV_powheg_pythia8/92X_LooseIsoPFTau20_83XSignalSamples/170620_145920/0000/outputFULL_669.root', parser.VarParsing.multiplicity.singleton,
...
#opts.register('globalTag', '80X_mcRun2_asymptotic_2016_TrancheIV_v6', parser.VarParsing.multiplicity.singleton,
#opts.register('globalTag', '80X_dataRun2_Prompt_ICHEP16JEC_v0', parser.VarParsing.multiplicity.singleton,
opts.register('globalTag', '92X_upgrade2017_TSG_For83XSamples_V4', parser.VarParsing.multiplicity.singleton,
#opts.register('globalTag', '92X_dataRun2_HLT_TrackAlignExtenErrors_w23_2017', parser.VarParsing.multiplicity.singleton,
...
#  if isData:
  ...
  process.icTriggerSequenceTEST += cms.Sequence(
    process.patTrigger+
    process.patTriggerEvent+
    process.icTriggerPathProducerTEST
  )
...
if release in ['80XMINIAOD']:
  ...
  process.icTriggerPathProducerTEST = producers.icTriggerPathProducer.clone(
   branch = cms.string("triggerPaths"),
   input  = cms.InputTag("TriggerResults","","HLTX"),
   inputIsStandAlone = cms.bool(True),
   inputPrescales = cms.InputTag("patTrigger")
  )
  ...
  if isReHLT:
    ...
    process.icTriggerPathProducerTEST.input = cms.InputTag("TriggerResults","","HLTX")
...
process.icVBF = producers.icTriggerObjectProducer.clone(
      input   = cms.InputTag("patTriggerEvent"),
      branch = cms.string("triggerVBF"),
      #hltPath = cms.string("HLT_VBF_ThreeJets_Reg_LooseChargedIsoPFTau20_Trk1_eta2p1_Reg_v1"),
      hltPath = cms.string("HLT_VBF_DoubleLooseChargedIsoPFTau20_Trk1_eta2p1_Reg_v1"),
      inputIsStandAlone = cms.bool(False),
      storeOnlyIfFired = cms.bool(False),
      inputTriggerResults = cms.InputTag("TriggerResults", "", "HLTX")
      )

process.icHLTDoubleMediumIsoPFTau35 = producers.icTriggerObjectProducer.clone(
      input   = cms.InputTag("patTriggerEvent"),
      branch = cms.string("triggerObjectsDoubleMediumIsoPFTau35"),
      hltPath = cms.string("HLT_DoubleMediumChargedIsoPFTau35_Trk1_eta2p1_Reg_v2"),
      inputIsStandAlone = cms.bool(False),
      storeOnlyIfFired = cms.bool(False),
      inputTriggerResults = cms.InputTag("TriggerResults", "", "HLTX")
      )

#process.icHLTDiJetVBFPassThroughProducer = producers.icTriggerObjectProducer.clone(
#      input   = cms.InputTag("patTriggerEvent"),
#      branch = cms.string("triggerObjectsDiJetVBFPassThrough"),
...
process.icTriggerObjectSequenceTEST += cms.Sequence(
      process.icVBF +
      process.icHLTDoubleMediumIsoPFTau35
      )

#process.icTriggerObjectSequence += cms.Sequence(
#      process.icHLTDiJetVBFPassThroughProducer
...
if release in ['80XMINIAOD']:
  for name in process.icTriggerObjectSequenceTEST.moduleNames():
    mod = getattr(process, name)
    mod.inputIsStandAlone = cms.bool(True)
    mod.input = cms.InputTag("selectedPatTriggerCustom", "","HLTX")
  if isReHLT:
    for name in process.icTriggerObjectSequenceTEST.moduleNames():
      mod = getattr(process, name)
      mod.inputTriggerResults = cms.InputTag("TriggerResults", "","HLTX")
...
#  process.icGenSequence+
  ...
  process.icTriggerSequenceTEST+
  process.icTriggerObjectSequenceTEST+
