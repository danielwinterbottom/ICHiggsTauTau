...
opts.register('file', 'file:root://xrootd.unl.edu//store/user/vmilosev/JetHT/crab_HTau/170509_164359/0000/outputTEST_1.root',
...
if release in ['80XMINIAOD']:
  ...
  if isReHLT:
    ...
    process.icTriggerPathProducerTEST.input = cms.InputTag("TriggerResults","","TEST")
...
process.icVBFPassThrough = producers.icTriggerObjectProducer.clone(
      input   = cms.InputTag("patTriggerEvent"),
...
process.icHbb = producers.icTriggerObjectProducer.clone(
      input   = cms.InputTag("patTriggerEvent"),
...
process.icHbbBS = producers.icTriggerObjectProducer.clone(
      input   = cms.InputTag("patTriggerEvent"),
      branch = cms.string("triggerObjectsHbbBS"),
      hltPath = cms.string("MolimTeNeRadi"),
      inputIsStandAlone = cms.bool(False),
      storeOnlyIfFired = cms.bool(False)
      )
...
if release in ['80XMINIAOD']:
  ...
  if isReHLT:
    for name in process.icTriggerObjectSequenceTEST.moduleNames():
      mod = getattr(process, name)
      mod.inputTriggerResults = cms.InputTag("TriggerResults", "","TEST")
