...
if release in ['72XMINIAOD']:
  ...
  process.elPFIsoValueChargedAll04PFIdPFIso = process.elPFIsoValueChargedAll03PFIdPFIso.clone()
  process.electronPFIsolationValuesSequence +=cms.Sequence(
...
if release in ['72XMINIAOD']:
  process.icElectronProducer.includePFIso04           = cms.bool(True) 
...
if release in ['72XMINIAOD']:
  ...
  process.muPFIsoValueChargedAll04PFIso = process.muPFIsoValueChargedAll03PFIso.clone()
  process.muonPFIsolationValuesSequence +=cms.Sequence(
...
if release in ['72XMINIAOD']: 
  ...
  process.icMuonProducer.includePFIso04 = cms.bool(True)
...
process.impactParameterTagInfosAK4PFCHS = btag.impactParameterTagInfos.clone(
...
process.secondaryVertexTagInfosAK4PFCHS = btag.secondaryVertexTagInfos.clone(
  trackIPTagInfos = cms.InputTag('impactParameterTagInfosAK4PFCHS')
)
process.simpleSecondaryVertexHighEffBJetTagsAK4PFCHS = btag.simpleSecondaryVertexHighEffBJetTags.clone (
  tagInfos = cms.VInputTag('secondaryVertexTagInfosAK4PFCHS')
)
process.simpleSecondaryVertexHighPurBJetTagsAK4PFCHS = btag.simpleSecondaryVertexHighPurBJetTags.clone (
  tagInfos = cms.VInputTag('secondaryVertexTagInfosAK4PFCHS')
)
process.combinedSecondaryVertexBJetTagsAK4PFCHS = btag.combinedSecondaryVertexBJetTags.clone (
...
process.icEle27GsfObjectProducer = producers.icTriggerObjectProducer.clone(
    ...
    hltPath = cms.string("HLT_Ele27_eta2p1_WP85_Gsf_v1"),
    inputIsStandAlone = cms.bool(False),
    storeOnlyIfFired = cms.bool(False)
    )
