#include "UserCode/ICHiggsTauTau/Analysis/HiggsNuNu/interface/Year1Proj.h"
...
namespace ic {
  ...
  Year1Proj::Year1Proj(std::string const& name): ModuleBase(name){
    fs_ = NULL;
    met_label_ = "pfMet";
  ...
  int  Year1Proj::PreAnalysis(){
    std::cout << "--------------------------------------------- " << std::endl
	      << "------ PreAnalysis Info for Year1Proj ------" << std::endl
	      << "--------------------------------------------- " << std::endl;
    if (fs_) {
    ...
    outputTree_ = fs_->make<TTree>("LightTree","Tree containing Year1ProjAna input variables");
...
//     outputTree_->Branch("jet2met_scalarprod",&jet2met_scalarprod_);
    ...
    outputTree_->Branch("mu1px",&mu1px_);
    outputTree_->Branch("mu1py",&mu1py_);
    outputTree_->Branch("mu1pz",&mu1pz_);
    outputTree_->Branch("mu1E",&mu1E_);

    outputTree_->Branch("mu2px",&mu2px_);
    outputTree_->Branch("mu2py",&mu2py_);
    outputTree_->Branch("mu2pz",&mu2pz_);
    outputTree_->Branch("mu2E",&mu2E_);

    return 0;
  }

  int  Year1Proj::Execute(TreeEvent *event){
    ...
    if(nselmuons_==2){
      ...
      mu1px_=selmuons.at(0)->vector().Px();
      mu1py_=selmuons.at(0)->vector().Py();
      mu1pz_=selmuons.at(0)->vector().Pz();
      mu1E_=selmuons.at(0)->vector().E();
      mu2px_=selmuons.at(1)->vector().Px();
      mu2py_=selmuons.at(1)->vector().Py();
      mu2pz_=selmuons.at(1)->vector().Pz();
      mu2E_=selmuons.at(1)->vector().E();
    }
  ...
  int  Year1Proj::PostAnalysis(){
    return 0;
  }

  void  Year1Proj::PrintInfo(){
    ;
  }
