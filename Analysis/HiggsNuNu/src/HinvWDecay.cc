...
namespace ic {
  ...
  HinvWDecay::HinvWDecay(std::string const& name, unsigned flavour, bool isEmbedded) : ModuleBase(name) {
    flavour_ = flavour;
  ...
  int HinvWDecay::Execute(TreeEvent *event) {
    ...
    if(do_newstatuscodes_){
          ...
          if(flags[IsPrompt]){
            ...
            if(abs(id)==15){
              ...
              if(notconsidered){
                ...
                }//endof Loop through tau decays until now tau found
              }//endof if notconsidered=true
            }//endof if abs(id)==15
          }
      ...
      for (unsigned i = 0; i < parts.size(); ++i) {//Loop on genparticles
        ...
        if ((!isEmbedded_ && (parts[i]->status() != 3&&!(parts[i]->status() >= 21&&parts[i]->status()<=29))) || (isEmbedded_ && parts[i]->status() != 2)) continue;
        ...
        if (id == 15) {//to find a tau status 3
          ...
          unsigned counter = 0;
          bool lDecay = false;
          for (unsigned j = 0; j < taus.size(); ++j) {//Loop on tau particles
            if ((!isEmbedded_ && (taus[j]->status() == 3||((taus[j]->status()) >= 21&&(taus[j]->status()<=29)))) || (isEmbedded_ && taus[j]->status() == 2 && fabs(taus[j]->pdgid())==15)){
              counter++;
