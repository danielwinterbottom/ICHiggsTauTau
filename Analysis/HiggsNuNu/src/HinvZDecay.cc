...
namespace ic {
  ...
  HinvZDecay::HinvZDecay(std::string const& name,unsigned flavour,double minMll,double maxMll) : ModuleBase(name) {
    flavour_ = flavour;
    min_ = minMll;
    max_ = maxMll;
  ...
  HinvZDecay::HinvZDecay(std::string const& name,unsigned flavour,double min,double max,cutVar variable,bool is2012) : ModuleBase(name) {
    flavour_ = flavour;
    min_ = min;
    max_ = max;
    countStatus3_ = 0;
  ...
  HinvZDecay::HinvZDecay(std::string const& name,unsigned flavour,double min,double max,bool variable,bool is2012) : ModuleBase(name) {
  ...
  int HinvZDecay::Execute(TreeEvent *event) {
    ...
    for (unsigned i = 0; i < parts.size(); ++i) {//Loop on genparticles
      ...
      if (!is2012_ && !(flags[GenStatusBits::IsHardProcess] && flags[GenStatusBits::FromHardProcess] && flags[GenStatusBits::IsFirstCopy] && (id==23 || abs(id)==11 || abs(id)==13 || abs(id) == 15))) continue;

      if (is2012_ && (parts[i]->status() != 3) && !(parts[i]->status() >= 21&&parts[i]->status() <= 29)) continue;
      ...
      if ( (is2012_ && id == static_cast<int>(flavour_)) || (!is2012_ && (id==11 || id==13 || id==15)) ) {
