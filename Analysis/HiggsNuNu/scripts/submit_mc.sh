...
#for FILELIST in `ls filelists/$QUEUEDIR/Dec30_MC_53X_DY*`
		...
		for FLAVOUR in enu munu taunu
		  ...
		  $JOBWRAPPER "./bin/HiggsNuNu --cfg=$CONFIG --filelist="$FILELIST" --input_prefix=$PREFIX --output_name=$WJOB.root --output_folder=$OUTPUTDIR --met_cut=$METCUT --signal_region=$DOQCD $SYSTOPTIONS --channel=$CHANNEL &> $JOBDIR/$WJOB.log" $JOBDIR/$WJOB.sh
