...
CONFIG=scripts/DefaultCompareConfig.cfg
QUEUEDIR=short #medium #medium long

JOBDIRPREFIX=jobs_run1comparerun1metsigtightcut090615
JOBDIR=$JOBDIRPREFIX/
OUTPUTPREFIX=output_run1comparerun1metsigtightcut090615
...
for syst in "" #JESUP JESDOWN JERBETTER JERWORSE UESUP UESDOWN ELEEFFUP ELEEFFDOWN MUEFFUP MUEFFDOWN PUUP PUDOWN
  ...
  for channels in nunu munu enu taunu mumu #top #qcd
    ...
    if [ "$syst" = "" ]
	then
	$JOBWRAPPER "./bin/run1comparison --cfg=$CONFIG --channel=$channels -o $OUTPUTDIR$syst/$OUTPUTNAME &> $JOBDIR$syst/$JOB.log" $JOBDIR$syst/$JOB.sh
    else
	if [ "$channels" = "nunu" ]
	    then
	    #for other systs only run nunu
	    $JOBWRAPPER "./bin/run1comparison --cfg=$CONFIG --channel=$channels --syst=$syst -o $OUTPUTDIR$syst/$OUTPUTNAME &> $JOBDIR$syst/$JOB.log" $JOBDIR$syst/$JOB.sh
	fi
    fi
    if [ "$syst" = "" ]
	then
