...
int main(int argc, char* argv[]){
  ...
  std::string data_pu_path = "input/pileup/Data_Pileup_2011_HCP-500bins.root";
  std::string mc_pu_path = "input/pileup/MC_Fall11_PU_S6-500bins.root";
  if (era == "2012A" || era=="2012B" || era=="2012")
  {
      data_pu_path = "input/pileup/Data_Pileup_2012_ReReco-600bins.root";
      mc_pu_path = "input/pileup/MC_Summer12_PU_S10-600bins.root";
  }
  TH1D data_pu  = GetFromTFile<TH1D>(data_pu_path, "/", "pileup");
  TH1D mc_pu    = GetFromTFile<TH1D>(mc_pu_path, "/", "pileup");
 
  std::string data_json;
  if (era == "2012A" || era == "2012B" || era == "2012C" || era == "2012D") data_json = "input/json/data_2012_moriond.txt" ;
  else data_json           =  "input/json/json_data_2011.txt";
