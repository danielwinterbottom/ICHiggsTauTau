#include "UserCode/AGilbert/interface/EventInfo.hh"
#include "UserCode/AGilbert/interface/PileupInfo.hh"
#include "UserCode/AGilbert/Analysis/interface/TreeEvent.h"
#include "UserCode/AGilbert/Modules/interface/ModuleBase.h"
#include <string>
#include "TH1F.h"
#include "TFile.h"

using namespace agilbert;

int PreAnalysis(TH1* data_, TH1* mc_, TH1* weights_, bool print_weights_) {
//    TH1* weights_;
//    bool print_weights_=true;
    unsigned nbins_data = data_->GetNbinsX();
    unsigned nbins_mc = mc_->GetNbinsX();
    double min_data = data_->GetBinLowEdge(1);
    double min_mc = mc_->GetBinLowEdge(1);
    double max_data = data_->GetBinLowEdge(nbins_data) + data_->GetBinWidth(nbins_data);
    double max_mc = mc_->GetBinLowEdge(nbins_mc) + mc_->GetBinWidth(nbins_mc);
    unsigned nbins = nbins_data > nbins_mc ? nbins_data : nbins_mc;
    std::cout << "----------------------------------------" << std::endl;
    std::cout << "PreAnalysis Info for PileupReweight" << std::endl;
    std::cout << "----------------------------------------" << std::endl;
    if (data_ && mc_) {
      std::cout << "- Input data histogram has " << nbins_data << " bins in the range [" << min_data << "," << max_data << "]" << std::endl;
      std::cout << "- Input MC histogram has " << nbins_mc << " bins in the range [" << min_mc << "," << max_mc << "]" << std::endl;
      std::cout << "- Weights calculated for " << nbins << " bins" << std::endl;
    } else {
      std::cout << "Invalid histogram!" << std::endl;
    }
    //weights_.resize(nbins);
    data_->Scale(1./data_->Integral());
    mc_->Scale(1./mc_->Integral());
    weights_ = (TH1*)data_->Clone();
    weights_->Divide(mc_);
    for (unsigned i = 0; i < nbins; ++i) {
      if (print_weights_) std::cout << "nInt = [" << weights_->GetBinLowEdge(i+1) << "," << weights_->GetBinLowEdge(i+2) << "],\tData = " << data_->GetBinContent(i+1) << ",  MC = " << mc_->GetBinContent(i+1) << ",  Weight = " << weights_->GetBinContent(i+1) << std::endl;
    }
    return 0;
  }

/*  int Execute(TreeEvent* event, TH1* weights_) {
    EventInfo * eventInfo = event->GetPtr<EventInfo>("eventInfo");
    std::vector<PileupInfo *> const& puInfo = event->GetPtrVec<PileupInfo>("pileupInfo");
    float true_int = -1;
    for (unsigned i = 0; i < puInfo.size(); ++i) {
      if (puInfo[i]->bunch_crossing() == 0) true_int = puInfo[i]->true_num_interactions();
    }
    if (true_int < 0) {
      std::cout << "Warning: In-time true_num_interactions not found!" << std::endl;
      return 0;
    }
    //double weight = 0.0;
    int found_bin = weights_->FindBin(true_int);
    double weight = 1.0;
    if (found_bin >= 1 && found_bin <= weights_->GetNbinsX()) {
      weight = weights_->GetBinContent(found_bin);
    }
    double weight=1.0;
    eventInfo->set_weight("pileup", weight);
    return 0;
  }
*/

int main(){

    TFile* fdata;
    TFile* fmc;
    fdata = new TFile("Data_Pileup_2012_HCP-600bins.root", "r");
    fmc = new TFile("MC_Summer12_PU_S10-600bins.root", "r");

    TH1* weights;

    TH1 *data = (TH1*)fdata->Get("pileup");
    TH1 *mc = (TH1*)fmc->Get("pileup");

    PreAnalysis(data, mc, weights, 1); 
    
    return 0;
}
