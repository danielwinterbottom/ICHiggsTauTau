...
namespace ic {
  ...
  class Sample {
    ...
    public:
      // Sample();
      Sample(std::string const& name, std::string const& category, std::string const& selection, TH1F *shape);

      inline void name(std::string const& name) { name_ = name; }
      inline std::string name() const { return name_; }

      inline void selection(std::string const& selection) { selection_ = selection; }
      inline std::string selection() const { return selection_; }

      inline void category(std::string const& category) { category_ = category; }
      inline std::string category() const { return category_; }

      inline void rate(double const& rate) { rate_ = rate; }
      inline double rate() const { return rate_; }
      
      inline void error(double const& error) { error_ = error; }
      inline double error() const { return error_; }
      
      inline void shape(TH1F *shape) { shape_ = shape; }
      inline TH1F * shape() const { return shape_; }

      inline void info(std::string const& label, double const& value) { info_[label] = value; }
      inline double info(std::string const& label) const { 
          auto it = info_.find(label);
          if (it != info_.end()) {
            return it->second;
          } else {
            std::cerr << "No info with label " << label << " for sample [" << name_ << ", " << category_ << ", " << selection_ << "]" << std::endl;
            return 0.0;
          }
      }

      inline void scale(double const& sf) {
        rate_ *= sf;
        error_ *= sf;
        if (shape_) shape_->Scale(sf);
      }
  };
    ...
    public:
      ...
      inline Sample & Get(std::string file, std::string category, std::string selection) {
        ...
        if (it != input_shapes_.end()) {
          ...
          std::cerr << "Error: No shape for (" << file << ", " << category << ", " << selection << ")" << std::endl;
          throw;
        }
      }
