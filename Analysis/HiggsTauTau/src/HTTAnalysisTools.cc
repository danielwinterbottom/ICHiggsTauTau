...
namespace ic {
  ...
  HTTAnalysis::HTTAnalysis(ic::channel ch, std::string cat, std::string var, unsigned bkg_method, bool is_2012, int verbosity) :
    ...
    if (ch_ != channel::em) {
      boost::range::push_back(input_files_, std::vector<std::string>{
        "Special_3_Data",
        "DYJetsToLL",
        "DYJetsToLL-L",
        "DYJetsToLL-J",
  ...
  void HTTAnalysis::ParseParamFile(std::string const& file) {
    /* We parse the parameter file to load:
        - the lumi
        - the signal mass points to load
        - the event numbers and cross section for each sample
    */
    ...
    for (auto file : input_files_) {
      if (parser.ParamExists("XS_"+file) && parser.ParamExists("EVT_"+file)) {
        if (parser.GetParam<double>("XS_"+file) > 0.0) sample_lumi_[file] = parser.GetParam<double>("EVT_"+file) / parser.GetParam<double>("XS_"+file);
      }
    }
  }

  Sample::Sample(std::string const& name, std::string const& category, std::string const& selection, TH1F *shape) {
    name_ = name;
    category_ = category;
  ...
  void HTTAnalysis::LoadHistograms() {
    for (auto file : input_files_) {
      // The input file is folder + sample name + channel + year
      std::string input_filename = input_folder_+"/"+file+"_"+Channel2String(ch_)+"_"+yr_+".root";
      TFile *input = new TFile(input_filename.c_str());
      if (!input) {
        std::cerr << "Error in <HTTAnalysis::LoadHistograms>: Unable to open file " << input_filename << std::endl;
        continue;
      }
      for (auto cat : input_cats_) {
        for (auto sel : input_sels_) {
          if (!gDirectory->cd((cat+"_"+sel).c_str())) {
            std::cerr << "Error in <HTTAnalysis::LoadHistograms>: Unable to find directory " << cat+"_"+sel << std::endl;
            continue;
          }
          TH1F *hist = (TH1F*)(gDirectory->Get(variable_.c_str()));
          if (!hist) {
            std::cerr << "Error in <HTTAnalysis::LoadHistograms>: Unable to load histogram" << std::endl;
            continue;            
          }
          std::string key = file + "_" + cat + "_" + sel;
          Sample sample(file, cat, sel, (TH1F*)hist->Clone());
          sample.rate(Integral(sample.shape()));
          sample.error(Error(sample.shape()));
        }
      }
    }
  }

  void HTTAnalysis::NormaliseSamples() {
    for (auto file : input_files_) {
      for (auto cat : input_cats_) {
        for (auto sel : input_sels_) {
          auto lumi_it = sample_lumi_.find(file);
          if (lumi_it != sample_lumi_.end()) {
            this->Get(file, cat, sel).scale(lumi_ / lumi_it->second);
          } else {
            continue;
          }
        }
