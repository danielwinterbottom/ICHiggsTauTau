...
namespace ic {
  ...
  TauIDTriggerFilter::TauIDTriggerFilter(std::string const& name) : ModuleBase(name), channel_(channel::zee), mc_(mc::summer12_53X) {
  }

  TauIDTriggerFilter::~TauIDTriggerFilter() {
    ;
  }

  int TauIDTriggerFilter::PreAnalysis() {
    std::cout << "** PreAnalysis Info for TauIDTriggerFilter **" << std::endl;
  ...
  int TauIDTriggerFilter::Execute(TreeEvent *event) {
    ...
    if (is_data_) {
      ...
      for (unsigned i = 0; i < triggerPathPtrVec.size(); ++i) {
        ...
        if (channel_ == channel::mt) {
          ...
          //2012 Triggers
          if (run >= 190456 /* && run <= xxxxx */ && name.find("HLT_IsoMu24_eta2p1_v") != name.npos) path_found = true;          
        }
      ...
      if (channel_ == channel::mt) {
        ...
        if (run >= 190456 && run <= 193751) {
          ...
          leg1_filter = "hltL3crIsoL1sMu16Eta2p1L1f0L2f16QL3f24QL3crIsoFiltered10";
          leg2_filter = "";
        }
        if (run >= 193752/* && run <= xxxxx*/) {
          trig_obj_label = "triggerObjectsIsoMu24";
  ...
  int TauIDTriggerFilter::PostAnalysis() {
    return 0;
  }

  void TauIDTriggerFilter::PrintInfo() {
    ;
  }
}
