...
namespace ic {
  ...
  int HTTWeights::PreAnalysis() {
    ...
    if(scalefactor_file_!="" && !is_embedded_) {
        TFile f(scalefactor_file_.c_str());
        w_ = std::shared_ptr<RooWorkspace>((RooWorkspace*)gDirectory->Get("w"));;
        f.Close();

        if(strategy_ == strategy::smsummer16 || strategy_ == strategy::cpsummer16 || strategy_type == strategy::legacy16 || strategy_ == strategy::cpdecays16){
        ...
        if(do_trg_weights_ || do_idiso_weights_) {
          if(mc_==mc::mc2017 || mc_ == mc::mc2018){
            ...
            } else if(strategy_ == strategy::smsummer16 || strategy_ == strategy::cpsummer16 ||  strategy_type == strategy::legacy16 || strategy_ == strategy::cpdecays16) {
          ...
          if(do_tau_id_sf_ && strategy_ != strategy::smsummer16 && strategy_ != strategy::cpsummer16 &&  strategy_type != strategy::legacy16 &&  strategy_ != strategy::cpdecays16 && mc_!=mc::mc2017 && mc_ != mc::mc2018){
    ...
    if(embedding_scalefactor_file_!="" && scalefactor_file_!="" && is_embedded_) {
  ...
  int HTTWeights::Execute(TreeEvent *event) {
    ...
    if (do_tau_id_sf_ && channel_!= channel::em  && channel_!= channel::tpzee  && channel_!= channel::tpzmm && channel_!= channel::tpmt && channel_!= channel::tpem) {
      ...
      if (channel_ != channel::tt){
        ...
        if((strategy_==strategy::smsummer16 || strategy_ == strategy::cpsummer16 ||  strategy_type == strategy::legacy16 || strategy_ == strategy::cpdecays16) && gen_match_2 == 5){
        ...
        auto args_2 = std::vector<double>{pt_2,eta_2,decay_mode_2};
        if(mc_ == mc::summer16_80X){
          if (strategy_ == strategy::smsummer16 || strategy_ == strategy::cpsummer16 || strategy_type == strategy::legacy16 || strategy_ == strategy::cpdecays16) tau_sf_1  = (gen_match_1 == 5 && !is_embedded_) ? 0.95 : 1.0;  
        ...
        if(mc_ == mc::summer16_80X){
          if(strategy_ == strategy::smsummer16 || strategy_ == strategy::cpsummer16 || strategy_type == strategy::legacy16 || strategy_ == strategy::cpdecays16) tau_sf_2  = (gen_match_2 == 5 && !is_embedded_) ? 0.95 : 1.0;
    ...
    if (do_zpt_weight_){
      ...
      if((mc_ != mc::summer16_80X && mc_ != mc::mc2017 && mc_ != mc::mc2018)|| strategy_== strategy::smsummer16 || strategy_ == strategy::cpsummer16 || strategy_type == strategy::legacy16 || strategy_ == strategy::cpdecays16){
   ...
   if (do_tracking_eff_){
       ...
       if (strategy_ == strategy::cpsummer16 || strategy_type == strategy::legacy16 || strategy_ == strategy::cpdecays16) embed_trk_e = 0.98;
   ...
   if (do_trg_weights_) {
     if (channel_ == channel::et) {
       ...
       if (mc_ == mc::summer12_53X) {
         ...
         tau_trg_mc=1;
          ...
          if(scalefactor_file_==""){ 
             ...
             if(!do_cross_trg_ && do_single_lepton_trg_){
                 ...
                 if(mc_ == mc::summer16_80X && strategy_ != strategy::smsummer16 && strategy_ != strategy::cpsummer16 &&  strategy_type != strategy::legacy16 && strategy_ != strategy::cpdecays16){
                   if(e_iso<0.1){
         ...
         ele_trg*=0.991; // correct for HLT Zvtx
         ...
         auto args_1 = std::vector<double>{e_pt,e_eta,e_iso};  
         auto args_desy = std::vector<double>{e_pt,e_signed_eta};  
         ele_trg = fns_["e_trgEle32orEle35_desy_data"]->eval(args_desy.data());
         if(!is_embedded_) ele_trg_mc = fns_["e_trgEle32orEle35_desy_mc"]->eval(args_desy.data());
         else ele_trg_mc = fns_["e_trg_binned_embed"]->eval(args_1.data());
         ...
         auto args_3 = std::vector<double>{t_pt,t_signed_eta,t_phi,t_dm};  
         auto args_3_nophi = std::vector<double>{t_pt,t_signed_eta,t_dm};
         double ele_xtrg = fns_["e_trg_EleTau_Ele24Leg_desy_data"]->eval(args_desy.data());
         double ele_xtrg_mc = 1.0;

         // std::cout << "data trg SF " << ele_trg << "mc trg SF " << ele_trg_mc << std::endl;
    
         if(!is_embedded_) ele_xtrg_mc = fns_["e_trg_EleTau_Ele24Leg_desy_mc"]->eval(args_desy.data());
         else ele_xtrg_mc = fns_["e_trg24_embed"]->eval(args_2.data());
         ...
         if(e_pt<33.) xtrg_OR_sf = xtrg_et_sf;
         if(t_pt<35.) xtrg_OR_sf = single_e_sf;
         ...
         ele_trg*=1.0; // correct for HLT Zvtx
   ...
   //                mu_trg_mc = mt_antiiso1_trig_mc_->GetBinContent(mt_antiiso1_trig_mc_->GetXaxis()->FindBin(m_eta),mt_antiiso1_trig_mc_->GetYaxis()->FindBin(pt));
               ...
               if(do_single_lepton_trg_ && !do_cross_trg_){
                   ...
                   if(mc_ == mc::summer16_80X && strategy_ != strategy::smsummer16 && strategy_ != strategy::cpsummer16 &&  strategy_type != strategy::legacy16 && strategy_ != strategy::cpdecays16){
                   ...
                   } else if(mc_ == mc::summer16_80X && (strategy_ == strategy::smsummer16 || strategy_ == strategy::cpsummer16 || strategy_type == strategy::legacy16 || strategy_ == strategy::cpdecays16)){
                     ...
                     if(m_iso<0.15 || strategy_ == strategy::smsummer16 || strategy_ == strategy::cpsummer16 ||  strategy_type == strategy::legacy16 || strategy_ == strategy::cpdecays16){
            ...
            if (mc_ == mc::mc2017) {
              stringStream << "m_trg20";
            }
            else if (mc_ == mc::mc2018) {
              stringStream << "m_trgIsoMu20Xtau_desy";
            }
            double mu_xtrg = fns_[stringStream.str() + "_data"]->eval(args_2.data());
            double mu_xtrg_mc;
            if(is_embedded_) mu_xtrg_mc = fns_[stringStream.str() + "_embed"]->eval(args_2.data());
            else mu_xtrg_mc = fns_[stringStream.str() + "_mc"]->eval(args_2.data());
     ...
     /*    if(e_pt<1000){
      ...
      if(mc_ !=mc::spring15_74X && mc_ != mc::fall15_76X && mc_!=mc::spring16_80X && mc_ != mc::summer16_80X && mc_ != mc::mc2017 && mc_ != mc::mc2018){
       ...
       if (mc_ == mc::fall15_76X){
         ...
         if(tt_trg_iso_mode_==4) {
             ...
             if(mc_ == mc::summer16_80X){
               ...
               if(strategy_ == strategy::smsummer16 || strategy_ == strategy::cpsummer16 || strategy_type == strategy::legacy16 || strategy_ == strategy::cpdecays16) isoWP = "Tight";
               if(gm1_ == 5){ 
       ...
       if (mc_ == mc::fall15_76X){
         ...
         ele1_trg = fns_["e_trg_binned_data"]->eval(args_1.data());
         if(!is_embedded_) ele1_trg_mc = fns_["e_trg_binned_mc"]->eval(args_1.data());
         else ele1_trg_mc = fns_["e_trg_binned_embed"]->eval(args_1.data());
         ele2_trg = 1.0;
         ele2_trg_mc = 1.0;
   ...
   if (do_idiso_weights_ || do_id_weights_) {
     if (channel_ == channel::et) {
       ...
       if (mc_ == mc::summer12_53X) {
       ...
       } else if (mc_ == mc::summer16_80X && strategy_ != strategy::smsummer16 && strategy_ != strategy::cpsummer16 &&  strategy_type != strategy::legacy16 &&  strategy_ != strategy::cpdecays16 && !is_embedded_){
          auto args_1 = std::vector<double>{pt,e_signed_eta};
          auto args_2 = std::vector<double>{pt,e_signed_eta,e_iso};
         ...
         if(is_embedded_){
           ele_iso = fns_["e_iso_binned_embed_ratio"]->eval(args_1.data());
           ele_id = fns_["e_id_embed_ratio"]->eval(args_2.data());
         } else {
         ...
         if(is_embedded_){
           ...
           ele_iso = 1.0;
           ele_id = 1.0;
           ele_idiso = fns_["e_idiso_desy_ratio"]->eval(args_2.data());
           /* ele_iso = fns_["e_iso_binned_ratio"]->eval(args_1.data()); */
           /* ele_id = fns_["e_id_ratio"]->eval(args_2.data()); */
         }
         /* ele_idiso = ele_iso*ele_id; */
       ...
       if(mc_==mc::mc2017){
         ...
         /* weight *= (ele_id * ele_iso); //ele_idiso //(ele_id * ele_iso) */
         weight *= (ele_idiso); //ele_idiso //(ele_id * ele_iso)
         event->Add("idweight_1", ele_id);
         event->Add("idweight_2", double(1.0));
         event->Add("isoweight_1", ele_iso);
         event->Add("isoweight_2", double(1.0));
       ...
       if (mc_ == mc::summer12_53X) {
         ...
         auto args_2 = std::vector<double>{pt,m_signed_eta,m_iso};  
         if(is_embedded_){
           mu_id = fns_["m_id_embed_ratio"]->eval(args_1.data());
           mu_iso = fns_["m_iso_binned_embed_ratio"]->eval(args_2.data());
           mu_idiso = fns_["m_idiso_binned_embed_ratio"]->eval(args_2.data());
         } else{
       ...
       if (mc_ == mc::summer12_53X) {
         ...
         if(e_pt<100){
          ...
          if(!is_embedded_) m_idiso = fns_["m_looseiso_binned_ratio"]->eval(args_2_2.data())*fns_["m_id_ratio"]->eval(args_2_1.data());;
          if(is_embedded_) m_idiso = fns_["m_looseiso_binned_embed_ratio"]->eval(args_2_2.data())*fns_["m_id_embed_ratio"]->eval(args_2_1.data());
