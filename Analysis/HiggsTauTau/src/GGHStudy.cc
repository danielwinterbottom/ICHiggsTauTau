#include "UserCode/ICHiggsTauTau/Analysis/HiggsTauTau/interface/GGHStudy.h"
...
namespace ic {
  ...
  GGHStudy::GGHStudy(std::string const& name) : ModuleBase(name) {
    fs_ = NULL;
    apply_hres_weight_ = false;
  }
  ...
  int GGHStudy::Execute(TreeEvent *event) {
    ...
    // Find the Higgs and apply HRes pt-reweighting if required
    GenParticle const* higgs = nullptr;
    for (auto const& part : gen_particles) {
    ...
    if (apply_hres_weight_) wt *= pt_weight;
    hists_->Fill("h_pt", h_pt, wt);
    ...
    unsigned all_gen_jets = gen_jets.size();
    ic::erase_if(gen_jets, !boost::bind(MinDRToCollection<GenParticle *>, _1, taus, 0.5));
    unsigned cleaned_gen_jets = gen_jets.size();
    hists_->Fill("overlap_jets", all_gen_jets - cleaned_gen_jets, wt);

    ic::erase_if(gen_jets, !boost::bind(MinPtMaxEta, _1, 30.0, 4.7));
    unsigned kinematic_jets = gen_jets.size();
