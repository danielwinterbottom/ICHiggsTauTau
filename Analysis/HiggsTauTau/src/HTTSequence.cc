...
// if(is_data){
   ...
   if(channel != channel::wmnu) {
     HTTPairSelector httPairSelector = HTTPairSelector("HTTPairSelector")
       ...
       .set_gen_taus_label(is_embedded ? "genParticlesEmbedded" : "genParticles")
...
if(!is_data&&channel != channel::wmnu){
...
void HTTSequence::BuildTauSelection(){
 ...
 if (tau_scale_mode > 0 && (!moriond_tau_scale||strategy_type==strategy::fall15||strategy_type==strategy::mssmspring16||strategy_type==strategy::smspring16 || strategy_type == strategy::mssmsummer16)){
