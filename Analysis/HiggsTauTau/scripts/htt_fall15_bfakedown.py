...
if options.proc_bkg or options.proc_all:
  ...
  for sa in central_samples:
      ...
      nperjob = 60
      if scales:
        JSONPATCH= (r"'{\"job\":{\"filelist\":\"%(FILELIST)s_%(sa)s.dat\",\"sequences\":{\"em\":[\"scale_bfake_lo\"],\"et\":[\"scale_bfake_lo\"],\"mt\":[\"scale_bfake_lo\"],\"tt\":[\"scale_bfake_lo\"]}}, \"sequence\":{\"output_name\":\"%(JOB)s\"}}' "%vars());
        if 'TT-ext' in sa:
          nperjob = 40
      ...
      nfiles = sum(1 for line in open('%(FILELIST)s_%(sa)s.dat' % vars()))
      for i in range (0,int(math.ceil(float(nfiles)/float(nperjob)))) :
        os.system('%(JOBWRAPPER)s "./bin/HTT --cfg=%(CONFIG)s --json=%(JSONPATCH)s --offset=%(i)d --nlines=%(nperjob)d &> jobs/%(JOB)s-%(i)d_bfakedown.log" jobs/%(JOB)s-%(i)s_bfakedown.sh' %vars())
        os.system('%(JOBSUBMIT)s jobs/%(JOB)s-%(i)d_bfakedown.sh' % vars())
      file_persamp.write("%s %d\n" %(JOB, int(math.ceil(float(nfiles)/float(nperjob)))))
