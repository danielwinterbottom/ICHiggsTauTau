...
subdirs = ['TSCALE0PI_UP','TSCALE0PI_DOWN','TSCALE1PI_UP','TSCALE1PI_DOWN','TSCALE3PRONG_UP','TSCALE3PRONG_DOWN','TSCALE3PRONG1PI0_UP','TSCALE3PRONG1PI0_DOWN','MET_SCALE_UP','MET_SCALE_DOWN','MET_RES_UP','MET_RES_DOWN', 'MUSCALE_UP','MUSCALE_DOWN']

nfiles={}

def FindMissingFiles(outf, d, samp, chan):
  #files=fnmatch.filter(os.listdir('%(outf)s/%(d)s'%vars()),'%(samp)s_2018_%(chan)s_*'%vars())
  files= glob.glob('%(outf)s/%(d)s/%(samp)s_2018_%(chan)s_*'%vars())
...
for sa in sample_list:
  ...
  for ch in channel:
    files=glob.glob('%(outputf)s/%(sa)s_2018_%(ch)s_*.root'%vars()) 
    if len(files)>0:
