...
do
 ...
 $JOBSUBMIT jobs/$JOB.sh
  ...
  JOB=Special_3_"$i"_mt_2012
  $JOBWRAPPER "./bin/TauIDMeasurement --is_data=1 --cfg=$CONFIG --filelist="$FILELIST"_"$i".dat --channel=mt  --output_name=Special_3_Data_"$JOB".root \
...
do
  ...
  JOB="$i"_mt_2012
  $JOBWRAPPER "./bin/TauIDMeasurement --is_data=1 --is_embedded=1 --cfg=$CONFIG --filelist="$FILELIST"_"$i".dat --channel=mt --output_name=Embedded_$JOB.root \
  --input_prefix=$PREFIX &> jobs/$JOB.log" jobs/$JOB.sh
  $JOBSUBMIT jobs/$JOB.sh
done
...
do
 ...
 $JOBWRAPPER "./bin/TauIDMeasurement --ztautau_mode=1 --is_data=0 --cfg=$CONFIG --filelist="$FILELIST"_"$i".dat --channel=mt --output_name=$JOB.root \
 --input_prefix=$PREFIX &> jobs/$JOB.log" jobs/$JOB.sh
 $JOBSUBMIT jobs/$JOB.sh

 JOB=DYJetsToLL-LSoup_"$i"_mt_2012
 $JOBWRAPPER "./bin/TauIDMeasurement --faked_tau_selector=1 --ztautau_mode=2 --is_data=0 --cfg=$CONFIG --filelist="$FILELIST"_"$i".dat --channel=mt --output_name=$JOB.root \
 --input_prefix=$PREFIX &> jobs/$JOB.log" jobs/$JOB.sh
 $JOBSUBMIT jobs/$JOB.sh
 
 JOB=DYJetsToLL-JSoup_"$i"_mt_2012
 $JOBWRAPPER "./bin/TauIDMeasurement --faked_tau_selector=2 --ztautau_mode=2 --is_data=0 --cfg=$CONFIG --filelist="$FILELIST"_"$i".dat --channel=mt --output_name=$JOB.root \
 --input_prefix=$PREFIX &> jobs/$JOB.log" jobs/$JOB.sh
 $JOBSUBMIT jobs/$JOB.sh
done
