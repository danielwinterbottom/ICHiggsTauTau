...
################################################################################
  ...
  --x_axis_label="Hadronic Tau Mass [GeV]" --datacard="tau_modes" \
  --background_scheme="tau_modes" --draw_ratio=false --extra_pad=1.15 --use_htt_style=true \
...
./bin/HiggsTauTauPlot4 --cfg=scripts/new_plot_"$ANA"_"$YEAR".cfg --channel=em --set_alias="sel:1" \
  --method=8 --cat="inclusive" --var="pzeta(48,-140,60)" --extra_pad=1.5 $EM_BAND_ONLY \
