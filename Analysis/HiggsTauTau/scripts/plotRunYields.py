...
for i, run in enumerate(runs):
    if not prescale_ok[str(run)]:
        boxes.append(bad_trigger.DrawBox(float(i), yield_hist.GetYaxis().GetXmin(), float(i+1), yield_hist.GetYaxis().GetXmax()))

pads[1].cd()
bad_trigger2 = ROOT.TBox()
bad_trigger2.SetFillColorAlpha(ROOT.kRed, 0.3)
...
for i, run in enumerate(runs):
    if not prescale_ok[str(run)]:
        boxes.append(bad_trigger2.DrawBox(float(i), plot.GetAxisHist(pads[1]).GetMinimum(), float(i+1), plot.GetAxisHist(pads[1]).GetMaximum()))
...
# plot.DrawTitle(pads[0], 'HLT_IsoMu22_v', 1)
if args.min_lumi > 0.:
