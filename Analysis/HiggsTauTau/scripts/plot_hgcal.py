...
infileHTT = "output/%s_VBFHToTauTau_M-125-hgcal-pu%s_%s_0.root" % (TAG, PU[0], CHN[0])
infileDY = "output/%s_DYToLL-M-50-hgcal-pu%s_%s_0.root" % (TAG, PU[0], CHN[0])
...
cuts = 'abs(eta_2)>1.5 && abs(eta_2)< 3.0'
htt_tree.Draw("m_sv>>htt_msv", "(%s) * 1.0" % cuts)
dy_tree.Draw("m_sv>>dy_msv", "(%s) * wt" % cuts)
htt_tree.Draw("m_vis>>htt_mvis", "(%s) * 1.0" % cuts)
dy_tree.Draw("m_vis>>dy_mvis", "(%s) * wt" % cuts)
