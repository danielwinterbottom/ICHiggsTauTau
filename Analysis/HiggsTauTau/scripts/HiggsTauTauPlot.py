...
if options.analysis == 'sm':
    if options.channel == 'mt':
        ...
        if options.era in ['cpsummer17']: 
          ...
          cats['baseline'] = '(iso_1<0.15 && mva_olddm_tight_2>0.5 && antiele_2 && antimu_2 && !leptonveto && ((trg_etaucross&&pt_2>35)||(trg_singleelectron&&pt_1>28)))' 
...
if options.era == 'cpsummer16':
  ...
  if options.channel in ['mt','et']: 
      cats['0jet'] = '(n_jets==0 && n_bjets==0 && pt_2>30)'
      cats['dijet']='n_jets>=2 && mjj>300 && n_bjets==0 && pt_2>30'
      ...
      cats['boosted'] = '(!(%s) && !(%s) && n_bjets==0 && pt_2>30)' % (cats['0jet'], cats['dijet'])
    ...
    cats['0jet'] = '(n_jets==0 && pt_1>50)'
    cats['dijet']='n_jets>=2 && mjj>300 && pt_1>50'
    cats['dijet_boosted']='%s && pt_tt>200' % cats['dijet']
    cats['dijet_lowboost']='%s && pt_tt<200' % cats['dijet']
    cats['dijet_loosemjj_boosted']='%s && mjj<500 && pt_tt>150' % cats['dijet']
    cats['dijet_loosemjj_lowboost']='%s && mjj<500 && pt_tt<150' % cats['dijet']
    ...
    cats['boosted'] = '(!(%s) && !(%s) && pt_1>50)' % (cats['0jet'], cats['dijet'])
...
if options.channel == 'tt':
    # mva_highMjj_ggh =     '(IC_highMjj_July25_1_max_index==0)'
    # mva_highMjj_misc =    '(IC_highMjj_July25_1_max_index==1)'
    # mva_highMjj_qcd =     '(IC_highMjj_July25_1_max_index==2)'
    # mva_highMjj_qqh =     '(IC_highMjj_July25_1_max_index==3)'
    ...
    # cats['jetFakes_tight_highMjj'] =  '({} && n_jets>=2 && mjj>300 && m_sv>110 && m_sv<145)'.format(mva_highMjj_jetFakes)
    # cats['jetFakes_loose_highMjj'] =  '({} && n_jets>=2 && mjj>300 && !(m_sv>110 && m_sv<145))'.format(mva_highMjj_jetFakes)
...
if options.channel in ['et','mt']:
    ...
    # cats['ggh_tight_highMjj'] =  '({} && n_jets>=2 && mjj>500)'.format(mva_highMjj_ggh)
    if options.channel == 'et':
        cats['ggh_highMjj'] = '({} && n_jets>=2 && mjj>300 && IC_binary_Oct11_score>0.6)'.format(mva_highMjj_ggh)
        cats['qqh_highMjj'] = '({} && n_jets>=2 && mjj>300 && IC_binary_Oct11_score<0.6)'.format(mva_highMjj_ggh)
    else:
        cats['ggh_highMjj'] = '({} && n_jets>=2 && mjj>300 && IC_binary_Oct11_score>0.55)'.format(mva_highMjj_ggh)
        cats['qqh_highMjj'] = '({} && n_jets>=2 && mjj>300 && IC_binary_Oct11_score<0.55)'.format(mva_highMjj_ggh)
...
if options.era in ['smsummer16','cpsummer16']: # Remove the False when finished!!!!!
    if options.channel in ['et','mt'] and options.cat in ['boosted','vbf','dijet','dijet_lowboost','dijet_boosted',
            ...
            'ggh_highMjj','ggh_tight_highMjj','ggh_loose_highMjj','ggh_loose_boost_highMjj','ggh_tight_boost_highMjj','qqh_highMjj','misc_highMjj','qcd_highMjj','qqh_highMjj','tt_highMjj','zll_highMjj','ztt_highMjj','fake_highMjj','jetFakes_highMjj','zttEmbed_highMjj','jetFakes_tight_highMjj','jetFakes_loose_highMjj']:
...
if options.era in ['cpsummer17','tauid2017']:
    # ztt_samples = ['DYJetsToLL-LO','DYJetsToLL-LO-ext1','DYJetsToLL_M-10-50-LO','DYJetsToLL_M-10-50-LO-ext1']
...
if options.era in ['cpsummer17']: 
    sm_samples = { 
            'ggH_htt': ['GluGluHToTauTau_M-*','GluGluHToTauTau_M-*-ext'], 
...
#     systematics['syst_scale_j_corr_up'] = ('JESCORR_UP' , '_'+options.syst_scale_j_corr+'Up', 'wt', ['EmbedZTT'], False)
#     systematics['syst_scale_j_corr_down'] = ('JESCORR_DOWN' , '_'+options.syst_scale_j_corr+'Down', 'wt', ['EmbedZTT'], False)
# if options.syst_scale_j_uncorr != '':
#     systematics['syst_scale_j_uncorr_up'] = ('JESUNCORR_UP' , '_'+options.syst_scale_j_uncorr+'Up', 'wt', ['EmbedZTT'], False)
#     systematics['syst_scale_j_uncorr_down'] = ('JESUNCORR_DOWN' , '_'+options.syst_scale_j_uncorr+'Down', 'wt', ['EmbedZTT'], False)
...
if options.qcd_os_ss_ratio > 0:
    ...
    if options.analysis == 'sm':
      if options.channel == 'et':
          qcd_os_ss_ratio = 1.0
          if options.cat == '0jet':
            if options.era in ['cpsummer16','cpsummer17']: qcd_os_ss_ratio = 0.97
            else: qcd_os_ss_ratio = 1.0
          elif options.cat == 'boosted':
            if options.era in ['cpsummer16','cpsummer17']: qcd_os_ss_ratio = 1.61
          ...
          if options.cat == '0jet':
            if options.era in ['cpsummer16','cpsummer17']: qcd_os_ss_ratio = 1.15
            else: qcd_os_ss_ratio = 1.07
          elif options.cat == 'boosted':
            if options.era in ['cpsummer16','cpsummer17']: qcd_os_ss_ratio = 1.18
