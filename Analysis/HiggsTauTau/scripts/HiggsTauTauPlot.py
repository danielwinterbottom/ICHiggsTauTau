...
if options.analysis in ['sm','cpprod','cpdecay']:
    if options.channel == 'mt':
        ...
        if options.era in ['cpsummer17','cp18']:
          #cats['baseline'] = '(iso_1<0.15 && deepTauVsJets_medium_2>0.5 && deepTauVsEle_tight_2>0.5 && deepTauVsMu_tight_2>0.5 && !leptonveto && ((trg_mutaucross&&pt_2>32&&pt_1<25)||(trg_singlemuon&&pt_1>25)) && wt<2)'# tightVsEle for et channel
          cats['baseline'] = '(iso_1<0.15 && deepTauVsJets_medium_2>0.5 && deepTauVsEle_vvloose_2>0.5 && deepTauVsMu_tight_2>0.5 && !leptonveto && ((trg_mutaucross&&pt_2>32&&pt_1<25)||(trg_singlemuon&&pt_1>25)) && wt<2)' #vvlooseVsElectron for mt and tt
...
if options.syst_res_met != '':
    ...
    if '$NJET' in hist_name: 
     ...
     systematics['syst_res_met_up'] = ('MET_RES_UP' , '_'+hist_name+'Up', 'wt', ['QCD','jetFakes','EmbedZTT','TT','TTJ','TTT','VV','VVT','VVJ','ggH_hww125','qqH_hww125','ggH_hww','qqH_hww', 'W'], False)
     systematics['syst_res_met_down'] = ('MET_RES_DOWN' , '_'+hist_name+'Down', 'wt', ['QCD','jetFakes','EmbedZTT','TT','TTJ','TTT','VV','VVT','VVJ','ggH_hww125','qqH_hww125','ggH_hww','qqH_hww','W'], False) 
if options.syst_scale_met != '':
    ...
    if '$NJET' in hist_name: 
      ...
      systematics['syst_scale_met_up'] = ('MET_SCALE_UP' , '_'+hist_name+'Up', 'wt', ['QCD','jetFakes','EmbedZTT','TT','TTJ','TTT','VV','VVT','VVJ','ggH_hww125','qqH_hww125','ggH_hww','qqH_hww', 'W'], False)
      systematics['syst_scale_met_down'] = ('MET_SCALE_DOWN' , '_'+hist_name+'Down', 'wt', ['QCD','jetFakes','EmbedZTT','TT','TTJ','TTT','VV','VVT','VVJ','ggH_hww125','qqH_hww125','ggH_hww','qqH_hww', 'W'], False) 
...
if options.method in [17,18] and options.channel in ['et','mt','tt'] and options.analysis=='cpprod' and options.do_ff_systs:
  if options.channel in ['et','mt']:
    ...
    lt_systs['ff_%s_qcd_closure_syst' % options.channel] = 'wt_ff_us_qcd_syst_closure_'
    lt_systs['ff_%s_wjets_closure_syst' % options.channel] = 'wt_ff_us_wjets_syst_closure_'
    lt_systs['ff_%s_ttbar_closure_syst' % options.channel] = 'wt_ff_us_ttbar_syst_closure_'
  ...
  if options.channel in ['tt']:
    ...
    for njet in ['0','1','2']:
      tt_systs['ff_tt_qcd_closure_syst_njets%(njet)s' % vars()] = 'wt_ff_us_qcd_syst_closure_njets%(njet)s' % vars()
...
def TTBarEmbeddingSyst(ana,outfile,template_name):    
    if options.channel == 'zmm': nominal_hist = outfile.Get(nodename+'/EmbedZL')
    else: nominal_hist = outfile.Get(nodename+'/EmbedZTT')
...
def RunPlotting(ana, cat='',cat_data='', sel='', add_name='', wt='wt', do_data=True, samples_to_skip=[], outfile='output.root',ff_syst_weight=None):
    ...
    if options.method in [17] and options.channel != "em":
        ...
        if 'ZTT' not in samples_to_skip and options.embedding and systematic == 'default':
