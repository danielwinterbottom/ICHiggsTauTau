...
tau_eta = ROOT.TH1D("tau_eta","tau_eta",50,np.array(xbins))
...
pairtree.Draw("(tau1_eta)>>tau_eta","abs(tau1_type)==15&&tau1_pt>20")
muon_eta.Sumw2()
electron_eta.Sumw2()
tau_eta.Sumw2()

muon_eta.Scale(1./muon_eta.Integral())
electron_eta.Scale(1./electron_eta.Integral())
...
legend = plot.PositionedLegend(0.45, 0.10, 3, 0.015)
plot.Set(legend, NColumns=2)
tau_eta_graph.GetXaxis().SetTitle("Tau #eta")
tau_eta_graph.GetYaxis().SetTitle("a.u.")
tau_eta_graph.Draw("LAXIS")
...
canv3.SaveAs('tauetadist.pdf')
canv3.Print('tauetadist.png')
