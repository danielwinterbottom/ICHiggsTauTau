...
for wp in wps:
  ...
  loc = '%(cmssw_base)s/src/UserCode/ICHiggsTauTau/Analysis/HiggsTauTau/mvadm_ff_deeptauV2p1_2017_et_newttbar/' % vars()
...
# systematic shifts for ttbar
  ...
  func = GetFromTFile(loc+'fakefactor_fits_et_%(wp)s_2017.root:met_closure_ttbar_mc_fit' % vars())
  ...
  for dmtype in ['mvadm','mvadm_nosig','dm']:
    ...
    w.factory('expr::met_var_w_bounded("max(min(3.5,@0),-3.5)",met_var_w[0])' % vars())
    for njet in ['0','1','2']:
      func = GetFromTFile(loc+'fakefactor_fits_et_%(wp)s_2017.root:%(dmtype)s_met_closure_njet%(njet)s_wjets_fit' % vars())
      corr_str = str(func.GetExpFormula('p')).replace('x','@0').replace(',false','')
    ...
    for njet in ['0','1','2']:
      func = GetFromTFile(loc+'fakefactor_fits_et_%(wp)s_2017.root:%(dmtype)s_pt_1_closure_njet%(njet)s_wjets_fit' % vars())
      corr_str = str(func.GetExpFormula('p')).replace('x','@0').replace(',false','')
      w.factory('expr::et_%(dmname)s_%(wp)s_wjets_e_pt_corr_njet%(njet)s("%(corr_str)s",e_pt_bounded90)' % vars())
    w.factory('expr::et_%(dmname)s_%(wp)s_wjets_e_pt_corr("(@4<28) + (@4>28)*((@0==0)*(@1) + (@0==1)*(@2) + (@0>1)*(@3))",njets[0],et_%(dmname)s_%(wp)s_wjets_e_pt_corr_njet0,et_%(dmname)s_%(wp)s_wjets_e_pt_corr_njet1,et_%(dmname)s_%(wp)s_wjets_e_pt_corr_njet2, e_pt[0])' % vars())
  ...
  for dmtype in ['mvadm','mvadm_nosig','dm']:
    ...
    # OS/SS correction
    for njet in ['0','1']:
      func = GetFromTFile(loc+'fakefactor_fits_et_%(wp)s_2017.root:%(dmtype)s_osss_closure_njet%(njet)s_qcd_fit' % vars())
...
for wp in wps:
  ...
  loc = '%(cmssw_base)s/src/UserCode/ICHiggsTauTau/Analysis/HiggsTauTau/mvadm_ff_deeptauV2p1_2017_mt_newnew/' % vars()
