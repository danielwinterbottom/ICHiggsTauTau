...
if options.proc_bkg or options.proc_all:
  ...
  for sa in central_samples:
      ...
      nperjob = 60
      if scales:
        JSONPATCH= (r"'{\"job\":{\"filelist\":\"%(FILELIST)s_%(sa)s.dat\",\"sequences\":{\"em\":[\"scale_b_hi\"],\"et\":[\"scale_b_hi\"],\"mt\":[\"scale_b_hi\"],\"tt\":[\"scale_b_hi\"]}}, \"sequence\":{\"output_name\":\"%(JOB)s\"}}' "%vars());
        if 'TT-ext' in sa:
          nperjob = 40
      ...
      nfiles = sum(1 for line in open('%(FILELIST)s_%(sa)s.dat' % vars()))
      for i in range (0,int(math.ceil(float(nfiles)/float(nperjob)))) :
        os.system('%(JOBWRAPPER)s "./bin/HTT --cfg=%(CONFIG)s --json=%(JSONPATCH)s --offset=%(i)d --nlines=%(nperjob)d &> jobs/%(JOB)s-%(i)d_bup.log" jobs/%(JOB)s-%(i)s_bup.sh' %vars())
        os.system('%(JOBSUBMIT)s jobs/%(JOB)s-%(i)d_bup.sh' % vars())
      file_persamp.write("%s %d\n" %(JOB, int(math.ceil(float(nfiles)/float(nperjob)))))
