...
# Data
 echo "Data"
 $JOBWRAPPER "./bin/HiggsTauTau --cfg=$CONFIG --filelist=$FILELIST/Data_"$DATASET"_et_skim_filelist.dat  \
                                --channel=et  --output_name=Data_et_2012.root  &> jobs/Data_et_2012.log" jobs/Data_et_2012.sh
 $JOBSUBMIT jobs/Data_et_2012.sh
 $JOBWRAPPER "./bin/HiggsTauTau --cfg=$CONFIG --filelist=$FILELIST/Data_"$DATASET"_mt_skim_filelist.dat  \
                                --channel=mt  --output_name=Data_mt_2012.root  &> jobs/Data_mt_2012.log" jobs/Data_mt_2012.sh
 $JOBSUBMIT jobs/Data_mt_2012.sh

# Embedded
for j in "${TSCALE[@]}"
do
  echo "Embedded"
 $JOBWRAPPER "./bin/HiggsTauTau --cfg=$CONFIG --tau_scale_mode=$j --filelist=$FILELIST/Embedded_et_skim_filelist.dat \
                                --channel=et --is_embedded=true \
                                --output_name=Embedded_et_2012.root &> Embedded_et_2012_TSCALE_"$j".log" jobs/Embedded_et_2012_TSCALE_"$j".sh 
 $JOBSUBMIT jobs/Embedded_et_2012_TSCALE_"$j".sh
 $JOBWRAPPER "./bin/HiggsTauTau --cfg=$CONFIG --tau_scale_mode=$j --filelist=$FILELIST/Embedded_mt_skim_filelist.dat \
                                --channel=mt --is_embedded=true \
                                ---output_name=Embedded_mt_2012.root &> Embedded_mt_2012_TSCALE_"$j".log" jobs/Embedded_mt_2012_TSCALE_"$j".sh
 $JOBSUBMIT jobs/Embedded_mt_2012_TSCALE_"$j".sh 
done
