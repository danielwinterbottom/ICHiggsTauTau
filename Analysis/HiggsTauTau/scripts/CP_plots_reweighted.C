...
void GenCPPlots(std::string outfile_name, std::string channel, int cp_chan){
  ...
  //TFile *f1 = new TFile((dir_name+"VBFHToTauTau_M-125_tt_2016.root").c_str());
  ...
  h->SetName("even_1");
  h->Write();
  t1->Draw((plot_string+binning).c_str(),(cut_string_1+"*wt_cp_0p5/wt_cp_0").c_str(),"goff");
  h = (TH1D*)gDirectory->Get("h");
  h->SetName("odd_1");
  h->Write();
  t1->Draw((plot_string+binning).c_str(),(cut_string_1+"*1/wt_cp_0").c_str(),"goff");
  h = (TH1D*)gDirectory->Get("h");
  h->SetName("none_1");
  h->Clone()->Write();
  ...
  if(cp_chan>1){
    ...
    h->SetName("even_2");
    h->Write();
    t1->Draw((plot_string+binning).c_str(),(cut_string_2+"*wt_cp_0p5/wt_cp_0").c_str(),"goff");
    h = (TH1D*)gDirectory->Get("h");
    h->SetName("odd_2");
    h->Write();
    t1->Draw((plot_string+binning).c_str(),(cut_string_2+"*1/wt_cp_0").c_str(),"goff");
    h = (TH1D*)gDirectory->Get("h");
    h->SetName("none_2");
    h->Write();   
