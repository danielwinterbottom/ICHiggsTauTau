...
# background uncerts split by process
bkg_uncert=0.2
num_sub_up_tt=num_data.Clone()
num_sub_down_tt=num_data.Clone()
denum_sub_up_tt=denom_data.Clone()
denum_sub_down_tt=denom_data.Clone()


denum_sub_up_tt.Add(denom_VV,-1)
denum_sub_up_tt.Add(denom_TT,-1-bkg_uncert)
denum_sub_up_tt.Add(denom_ZLL,-1)

denum_sub_down_tt.Add(denom_VV,-1)
denum_sub_down_tt.Add(denom_TT,-1+bkg_uncert)
denum_sub_down_tt.Add(denom_ZLL,-1)

num_sub_up_tt.Divide(denum_sub_up_tt)
num_sub_down_tt.Divide(denum_sub_down_tt)

num_sub_up_vv=num_data.Clone()
num_sub_down_vv=num_data.Clone()
denum_sub_up_vv=denom_data.Clone()
denum_sub_down_vv=denom_data.Clone()


denum_sub_up_vv.Add(denom_VV,-1-bkg_uncert)
denum_sub_up_vv.Add(denom_TT,-1)
denum_sub_up_vv.Add(denom_ZLL,-1)

denum_sub_down_vv.Add(denom_VV,-1+bkg_uncert)
denum_sub_down_vv.Add(denom_TT,-1)
denum_sub_down_vv.Add(denom_ZLL,-1)

num_sub_up_vv.Divide(denum_sub_up_vv)
num_sub_down_vv.Divide(denum_sub_down_vv)

num_sub_up_z=num_data.Clone()
num_sub_down_z=num_data.Clone()
denum_sub_up_z=denom_data.Clone()
denum_sub_down_z=denom_data.Clone()


denum_sub_up_z.Add(denom_VV,-1)
denum_sub_up_z.Add(denom_TT,-1)
denum_sub_up_z.Add(denom_ZLL,-1-bkg_uncert)

denum_sub_down_z.Add(denom_VV,-1)
denum_sub_down_z.Add(denom_TT,-1)
denum_sub_down_z.Add(denom_ZLL,-1+bkg_uncert)

num_sub_up_z.Divide(denum_sub_up_z)
num_sub_down_z.Divide(denum_sub_down_z)
...
# this bit to add uncertainties in quadrature
for i in range(1,num_data.GetNbinsX()+1):
  error = num_data.GetBinError(i)
  #bkg_uncert=abs(num_sub_up.GetBinContent(i) - num_sub_down.GetBinContent(i))/2
  bkg_uncert=((num_sub_up_tt.GetBinContent(i) - num_sub_down_tt.GetBinContent(i))/2)**2 + ((num_sub_up_z.GetBinContent(i) - num_sub_down_z.GetBinContent(i))/2)**2 + ((num_sub_up_vv.GetBinContent(i) - num_sub_down_vv.GetBinContent(i))/2)**2
  bkg_uncert = math.sqrt(bkg_uncert)
  jes_uncert=abs(num_W_up.GetBinContent(i) - num_W_down.GetBinContent(i))/2
  num_data.SetBinError(i, math.sqrt(error**2+bkg_uncert**2+jes_uncert**2))
...
axish[0].Draw()
num_data.SetFillColor(16)
num_data.DrawCopy("E2PLSAME")
#num_sub_down.DrawCopy("LSAME")
#num_sub_up.DrawCopy("LSAME")
num_W.DrawCopy("E0LSAME")

legend = plot.PositionedLegend(0.3,0.2,3,0.03)
...
ratio_data_hist.SetFillColor(16)
ratio_data_hist.DrawCopy("PLSAME E2")
...
func_up= func.Clone()
func_up.SetParameter(1, func.GetParameter(1)  + func.GetParError(1))
func_up.SetLineStyle(2)
#func_up.SetLineColor(1)
func_up.Draw("LSAME")

func_down= func.Clone()
func_down.SetParameter(1, func.GetParameter(1)  - func.GetParError(1))
func_down.SetLineStyle(2)
#func_down.SetLineColor(1)

func_down.Draw("LSAME")

ratio_data_hist.SetLineColor(0)
legend.AddEntry(ratio_data_hist,"Uncertainty","f")
legend.AddEntry(func,"pol1 fit","l")
legend.AddEntry(func_up,"pol1 fit #pm 1#sigma","l")
legend.Draw("same")
...
c2.SaveAs("W_jettaufake_withuncerts.pdf")
