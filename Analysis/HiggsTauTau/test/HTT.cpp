...
//  analysis.DoSkimming("./skim/");
  ...
  for (unsigned i = 0; i < js["job"]["channels"].size(); ++i) {
    ...
    for (unsigned j = 0; j < vars.size(); ++j) {
      ...
      if((channel_str.find("mt") == channel_str.npos && channel_str.find("em") == channel_str.npos && channel_str.find("zmm") == channel_str.npos) && vars[j].find("scale_mu_")!=std::string::npos) continue;     
      ...
      if( (channel_str.find("em") != channel_str.npos || channel_str.find("zmm") != channel_str.npos) && (vars[j].find("scale_t_0pi")!=std::string::npos || vars[j].find("scale_t_1pi")!=std::string::npos || vars[j].find("scale_t_3prong")!=std::string::npos || vars[j].find("scale_t_3prong1pi0")!=std::string::npos )) continue;
