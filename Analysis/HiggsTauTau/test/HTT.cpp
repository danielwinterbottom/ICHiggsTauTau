...
//  analysis.DoSkimming("./skim/");
  ...
  for (unsigned i = 0; i < js["job"]["channels"].size(); ++i) {
    ...
    for (unsigned j = 0; j < vars.size(); ++j) {
      ...
      if((channel_str.find("zmm") == channel_str.npos && channel_str.find("mt") == channel_str.npos && channel_str.find("em") == channel_str.npos) 
         && vars[j].find("scale_mu_")!=std::string::npos) continue;     
