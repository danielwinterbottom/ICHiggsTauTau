...
TGraph* ROCCurve(TTree *tree, std::string gen_sel, std::string sel, std::string variable){
  ...
  std::string real_sel = sel+"*wt*"+gen_sel;
  std::string fake_sel = sel+"*wt*(gen_match!=1 && gen_match!=3 && gen_match!=7)";
...
int main(int /*argc*/, char* /*argv*/[]){
  ...
  std::string file_name = "/vols/cms/dw515/Offline/output/DY_ID/DYJetsToLL_et_2016.root";  
  ...
  std::string hadE_sel = "(gen_match==7)";
  ...
  for(it_type iterator = det_region_.begin(); iterator != det_region_.end(); iterator++){
    ...
    TGraph *hadE_roc_2015 = ROCCurve(tree, hadE_sel, sel,"electronmvaID");
    TGraph *hadE_roc_2016 = ROCCurve(tree, hadE_sel, sel,"id_2016");
    ...
    std::vector<TGraph*> hadE_comp;
    std::vector<std::string> hadE_comp_leg;
    hadE_comp.push_back((TGraph*)hadE_roc_2016->Clone());
    hadE_comp_leg.push_back("GeneralPurpose MVA 2016");
    hadE_comp.push_back((TGraph*)hadE_roc_2015->Clone());
    hadE_comp_leg.push_back("Non-Triggering MVA 2015");
