...
int main(int argc, char* argv[]){
 ...
 // 09 dilepton!!
  ...
  if (mssm_mode == 0) {
    comp_plots.push_back("ggHTohhTo2Tau2B300");
  } else {
    comp_plots.push_back("ggH160");
    comp_plots.push_back("bbH160");
  }
  ...
  if (mssm_mode == 0) {
    ...
    std::cout << boost::format("%-25s %-9.2f %-9.2f %-9.2f %-9.2f %-9.2f %-9.2f %-9.2f %-9.2f %-9.2f %-9.2f %-9.2f %-9.2f\n") 
         % (category)  % ("data_obs") % ("ZTT") % ("W") % ("QCD")
                % ("TT") % ("VV") % ("ZLL") % ("ZJ") % ("ZL") % ("TOT") % ("DATA/MC") % ("ggHTohh300");
    } else {
  ...
  for (unsigned i = 0; i < inputs.size(); ++i) {
    ...
    if (mode !=2) {
      ...
      ggh_yield[i] =       (TH1F*)gDirectory->Get("ggHTohhTo2Tau2B300") ? Integral((TH1F*)gDirectory->Get("ggHTohhTo2Tau2B300")) : 0;
      ...
      ggh_yield_err[i] =       (TH1F*)gDirectory->Get("ggHTohhTo2Tau2B300") ? Error((TH1F*)gDirectory->Get("ggHTohhTo2Tau2B300")) : 0;
    } else {
      data_obs_yield[i] = (TH1F*)gDirectory->Get("data_obs") ? Integral((TH1F*)gDirectory->Get("data_obs")) : 0;
      ztt_yield[i] =      (TH1F*)gDirectory->Get("Ztt") ? Integral((TH1F*)gDirectory->Get("Ztt")) : 0;
    ...
    if (mssm_mode == 0) {
      qqh_yield[i] =       (TH1F*)gDirectory->Get("ggHTohhTo2Tau2B300") ? Integral((TH1F*)gDirectory->Get("ggHTohhTo2Tau2B300")) : 0;
      ggh_yield_err[i] =       (TH1F*)gDirectory->Get("ggHTohhTo2Tau2B300") ? Error((TH1F*)gDirectory->Get("ggHTohhTo2Tau2B300")) : 0;
