...
int main(int argc, char* argv[]){
  ...
  // Special Mode 25 No ID or Iso or muon or electron leg (for skimming only)
    - Need to be able to shift the mass (m_sv and m_vis) in the embedded samples up by
      1% - this could be done easily in HhhCategories
...
//  }
  ...
  // Category Modules
  // ------------------------------------------------------------------------------------  
  HhhCategories hhhCategories = HhhCategories("HhhCategories")
    .set_fs(fs)
    .set_channel(channel)
    .set_era(era)
    .set_strategy(strategy)
  ...
  if (mass_scale_mode == 3) hhhCategories.set_mass_shift(1.02);
