...
namespace ic {
  ...
  HhhPlot::HhhPlot() : config_("Config") {
    bkg_schemes_["all"] = {
...
*/
  ...
  void HhhPlot::SetMCStackStyle(ic::TH1PlotElement & ele, unsigned color) {
  ...
  void HhhPlot::SetSignalStyle(ic::TH1PlotElement & ele, unsigned color) {
    ele.set_fill_style(1001);
    ele.set_draw_fill(true);
    ele.set_draw_marker(false);
    ele.set_draw_line(false);
  ...
  void HhhPlot::SetDataStyle(ic::TH1PlotElement & ele) {
  ...
  void HhhPlot::SetRatioStyle(ic::RatioPlotElement & ele, unsigned color) {
  ...
  boost::program_options::options_description const& HhhPlot::GenerateOptions(std::string const& prefix) {
  ...
  void HhhPlot::GeneratePlot(HhhAnalysis::HistValueMap hmap) {
    ...
    sig_schemes_["mssm_1pb"] = {
      PlotSigComponent("sig",
        ("1pb hh#rightarrow#tau#taubb(m_{H}="+draw_signal_mass_+")"),
        {"ggHTohhTo2Tau2B"}, kBlue+3, false),
      PlotSigComponent("sig_AZh",
        ("1pb Zh#rightarrow#tau#taubb(m_{A}="+draw_signal_mass_+")"),
        {"ggAToZhToLLBB","ggAToZhToTauTauBB","ggAToZhToBBTauTau"}, kGreen+3, false)
    };
    sig_schemes_["mssm_highmass"] = {
      PlotSigComponent("sig_Radion",
        ("1pb Radion hh#rightarrow#tau#taubb(m_{H}="+draw_signal_mass_+")"),
        {"RadionToHH"}, kBlue+3, false),
      PlotSigComponent("sig_Graviton",
        ("1pb Graviton hh#rightarrow#tau#taubb(m_{H}="+draw_signal_mass_+")"),
    ...
    for (unsigned i = 0; i < bkg_scheme.size(); ++i) {
      ...
      if (valid_element) {
        HhhPlot::SetMCStackStyle(bkg_elements.back(), bkg_scheme[i].color);
    ...
    for (unsigned i = 0; i < sig_scheme.size(); ++i) {
      ...
      HhhPlot::SetSignalStyle(sig_elements.back(), sig_scheme[i].color);
  ...
  void HhhPlot::AddTextElement(ic::TextElement & ele) {
    text_.push_back(ele);
  }
