...
def FindMissingFiles(outf, d, samp, chan, infiles):
  files=fnmatch.filter(infiles,'%(samp)s_2016_preVFP_%(chan)s_*'%vars())
  nums = [int(x.split('_')[-1].replace('.root','')) for x in files]
  nums.sort()
  res = [ele for ele in range(max(nums)+1) if ele not in nums]

  if len(res) !=0:
    print "Some files are missing for sample %(samp)s_2016_preVFP_%(chan)s! in %(d)s:"%vars()
    for x in res: print '%(samp)s_2016_preVFP_%(chan)s_%(x)i.root' % vars()
    return False
  else:
    return True
...
#          files=glob.glob('%(outputf)s/%(sdir)s/%(sa)s_2016_preVFP_preVFP_%(ch)s_*.root'%vars())
          ...
          if no_missing_files and (ignore ==True or len(fnmatch.filter(infiles,'%(sa)s_2016_preVFP_%(ch)s_*'%vars())) == nfiles["%(sa)s_2016_preVFP"%vars()]):
            if not batch:  
              ...
              os.system('hadd -f %(outputf)s/%(sdir)s/%(sa)s_%(ch)s_2016_preVFP.root %(outputf)s/%(sdir)s/%(sa)s_2016_preVFP_%(ch)s_* &> ./haddout.txt'% vars()) 
