...
parser.add_option("--cfg", dest="config", type='string', default='scripts/plot_cpdecays_leg2016.cfg',
                  help="The config file that will be passed to HiggsTauTauPlotOld.py. Parameter file, input folder and signal scheme taken from this cfg file unless overriden by command line options")
...
if SCHEME == 'cpdecay':
  ...
  # VAR_PIPI ="IC_11May2020_max_score,aco_angle_6[0.,0.6,0.8],(6,0,6.28319)"
  # #VAR_PIRHO ="IC_11May2020_max_score,aco_angle_5[0.,0.65,0.75,0.85],(14,0,6.28319)" # could use even more bins (20)!
  # VAR_PIRHO ="IC_11May2020_max_score,aco_angle_5[0.,0.65,0.75,0.85],(16,0,6.28319)" # could use even more bins (20)!
    ...
    for x in scheme:
        ...
        if not options.hadd:
            if not options.batch:
                print('python $CMSSW_BASE/src/UserCode/ICHiggsTauTau/Analysis/HiggsTauTauRun2/scripts/HiggsTauTauPlotOld.py --cfg=%(CFG)s --channel=%(ch)s --method=%(cat_num)s --cat=%(cat_str)s --year=%(YEAR)s --outputfolder=%(output_folder)s --datacard=%(dc)s --paramfile=%(PARAMS)s --folder=%(FOLDER)s %(BLIND)s --var="%(var)s" %(extra)s --no_plot' % vars())
                os.system('python $CMSSW_BASE/src/UserCode/ICHiggsTauTau/Analysis/HiggsTauTauRun2/scripts/HiggsTauTauPlotOld.py --cfg=%(CFG)s --channel=%(ch)s'
