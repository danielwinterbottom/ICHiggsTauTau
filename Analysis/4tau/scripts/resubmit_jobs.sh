...
for i in $(ls jobs/*.sh); do 
  ...
  job=$i
  i=${job//.sh/".log"}
  if grep -q "End of job" $i && ! grep -q "Disk quota exceeded" $i && ! grep -q "Processing Complete: 0 events processed" $i; then
    continue;
  else 
...
source /vols/grid/cms/setup.sh' $job
    ...
    (( COUNT++ ))
    if [[ $1 != "" ]]; then
      #qsub -e /dev/null -o /dev/null -V -q hep.q -pe hep.pe 4 -l h_rt=0:$1:0 -l h_vmem=24G -cwd $job
