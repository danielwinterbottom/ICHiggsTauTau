...
# python scripts/htt_UL_2017.py --sig --bkg --data --jetmetuncerts --scales="default,scale_t_0pi,scale_t_1pi,scale_t_3prong,scale_t_3prong1pi0,scale_efake_0pi,scale_efake_1pi,scale_mufake_0pi,scale_mufake_1pi,scale_e" --submit='./scripts/submit_ic_batch_job.sh "hep.q -l h_rt=0:180:0 -l h_vmem=24G"' --parajobs
...
parser.add_option("--config", dest="config", type='string', default='scripts/config_UL_2017.json',
                  help="Config file")
...
CONFIG='scripts/config_UL_2017.json'
...
if options.proc_data or options.proc_all or options.calc_lumi:
  if not no_json:
    ...
    channels=["tttt","mttt","ettt","emtt","eett","mmtt","mmmm"]
...
if options.proc_bkg or options.proc_all:
  ...
  for sa in central_samples:
      ...
      for FLATJSONPATCH in flatjsons:
        nperjob = 100
        if "DY" in sa or "TTTo" in sa or 'GluGluToContinToZZTo4mu' in sa: nperjob = 40
