#include "UserCode/ICHiggsTauTau/Analysis/4tau/interface/HTTThreeTauGenInfo.h"
...
namespace ic {
  ...
  HTTThreeTauGenInfo::HTTThreeTauGenInfo(std::string const& name) : ModuleBase(name), channel_(channel::mt) {
    ditau_label_ = "ditau";
    ngenjets_=false;
  }

  HTTThreeTauGenInfo::~HTTThreeTauGenInfo() {
    ;
  }

  int HTTThreeTauGenInfo::PreAnalysis() {
    return 0;
  }

  int HTTThreeTauGenInfo::Execute(TreeEvent *event) {
  ...
  int HTTThreeTauGenInfo::PostAnalysis() {
    return 0;
  }

  void HTTThreeTauGenInfo::PrintInfo() {
    ;
  }
