...
h1 = f1.Get('jet_energy_cont_profile')
h2 = f2.Get('jet_energy_cont_profile')


h1.Draw("axis")
axis = plot.GetAxisHist(pads[0])
axis.GetXaxis().SetTitle('#DeltaR')
axis.GetYaxis().SetTitle('<E_{T}(R < #DeltaR) / E_{T}^{jet}(R=0.8)>')
axis.SetMaximum(0.15)

pr1 = h1.ProfileX('h1', 1, -1, 's')
pr2 = h2.ProfileX('h2', 1, -1, 's')
pr1.SetMarkerSize(0.2)

plot.Set(pr1, MarkerSize=0.5, LineWidth=2, MarkerColor=ROOT.kBlue, LineColor=ROOT.kBlue)
plot.Set(pr2, MarkerSize=0.5, LineWidth=2, MarkerColor=ROOT.kRed, LineColor=ROOT.kRed)

pr1.Draw('SAMEE')
pr2.Draw('SAMEE')

legend.AddEntry(pr1, '#tau#rightarrow#pi^{+}#pi^{0} (0 PU)', 'L')
legend.AddEntry(pr2, 'Jet#rightarrow#tau fakes (0 PU)', 'L')
legend.Draw()

latex = ROOT.TLatex()
latex.SetNDC()
latex.SetTextSize(0.04)
latex.DrawLatex(0.6, 0.4, '40 < p_{T}^{jet} < 60 GeV')
latex.DrawLatex(0.6, 0.35, '2.0 < |#eta^{jet}| < 2.5')
