...
namespace ic
{
// 	double getPV_angle_test(TLorentzVector Tauminus, std::vector<TLorentzVector> pis_1, std::vector<double> charges_1, TLorentzVector Tauplus, std::vector<TLorentzVector> pis_2, std::vector<double> charges_2)
// 	{
...
// 	}
// 	
// 	double getPV_angle_pola_test(std::vector<TLorentzVector> pis_1, std::vector<TLorentzVector> pis_2,
// 	                        TVector3 sv_1, TVector3 sv_2,
// 	                        double met_x, double met_y)
// 	{//in case of a1a1 decay we produce the polarimetric vector angle, this updated method includes the shift of 'unphysical taus'
...
//   }
	...
	double getIPPV_angle_test(std::vector<TLorentzVector> listPionsA1,
    ...
    // we always have a1 first
	{
		...
		double angle_test = -9999;
		//this performs the O shift
		if(((h1.Cross(h2))*(tauA1_HRF.Unit()))<=0){
		    angle_test = TMath::ATan2((k1.Cross(k2)).Mag(),k1*k2);}
		else{
		    angle_test = (2.*TMath::Pi()-TMath::ATan2((k1.Cross(k2)).Mag(),k1*k2));}
		
		//in case of an a1-rho decay we need to perform the y shift for polarisation states
		if ((IsRho) && (!isnan(angle_test))){
		    double y = (IP_ZMF.E() - pi_ZMF.E()) / (IP_ZMF.E() + pi_ZMF.E());
		    if (y < 0 ){
				if (angle_test < TMath::Pi()){
				    angle_test = angle_test + TMath::Pi();
				}
				else {
				    angle_test = angle_test - TMath::Pi();
				}
