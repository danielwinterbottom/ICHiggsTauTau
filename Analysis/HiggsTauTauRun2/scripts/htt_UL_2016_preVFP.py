...
parser.add_option("--config", dest="config", type='string', default='scripts/config_UL_2016_preVFP.json',
                  help="Config file")
...
CONFIG='scripts/config_UL_2016_preVFP.json'
...
if options.proc_embed or options.proc_all:
    ...
    embed_samples = []
    data_eras = ['B_ver2','C','D','E','F']
    for chn in channels:
        for era in data_eras:
            if 'em' in chn:
                embed_samples+=['EmbeddingElMu2016-HIPM_'+era]
            if 'et' in chn:
                embed_samples+=['EmbeddingElTau2016-HIPM_'+era]
            if 'mt' in chn:
                embed_samples+=['EmbeddingMuTau2016-HIPM_'+era]
            if 'tt' in chn:
                embed_samples+=['EmbeddingTauTau2016-HIPM_'+era]
            if 'zmm' in chn:
                embed_samples+=['EmbeddingMuMu2016-HIPM_'+era]
            if 'zee' in chn:
                embed_samples+=['EmbeddingElEl2016-HIPM_'+era]

    EMBEDFILELIST="./filelists/May25_2016-preVFP_MC_124X"

    for sa in embed_samples:
        job_num=0
        JOB='%s_2016preVFP' % (sa)
        JSONPATCH= (r"'{\"job\":{\"filelist\":\"%(EMBEDFILELIST)s_%(sa)s.dat\",\"file_prefix\":\"root://gfe02.grid.hep.ph.ic.ac.uk:1097//store/user/dwinterb/May25_EMB_124X_2016-preVFP/\",\"sequences\":{\"em\":[],\"et\":[],\"mt\":[],\"tt\":[],\"zmm\":[],\"zee\":[]}}, \"sequence\":{\"output_name\":\"%(JOB)s\",\"is_embedded\":true,%(jetuncert_string)s}}' "%vars());
...
#     'QCD_Pt-80To120_MuEnrichedPt5',
  ...
  for sa in central_samples:
      ...
      if 'MuEnrichedPt5' in sa:
        PREFIX='May25_MC_124X_2016-preVFP'
        FILELIST='filelists/May25_2016-preVFP_MC_124X'
      elif 'MuEnrichedPt15' in sa:
        PREFIX='Apr20_MC_124X_2016-preVFP'
        FILELIST='filelists/Apr20_2016-preVFP_MC_124X'
      else:
        PREFIX='Mar31_MC_124X_2016-preVFP'
        FILELIST='filelists/Mar31_2016-preVFP_MC_124X'
