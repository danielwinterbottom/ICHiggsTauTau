...
import argparse
import ConfigParser
import UserCode.ICHiggsTauTau.plotting_w as plotting
...
if options.analysis in ['sm','cpprod','cpdecay']:
    if options.channel == 'mt':
        ...
        if options.era in ['cpsummer17','cp18']:
          cats['baseline'] = '(iso_1<0.15 && deepTauVsJets_medium_2>0.5 && deepTauVsEle_vvloose_2>0.5 && deepTauVsMu_tight_2>0.5 && leptonveto==0 && ((trg_mutaucross&&pt_1<25&&pt_2>32)||(trg_singlemuon&&pt_1>25)) && wt<2 && n_deepbjets==0 && pt_2>30)'
...
if options.syst_e_scale != '':
    systematics['scale_e_up'] = ('ESCALE_UP' , '_'+options.syst_e_scale+'Up', 'wt', [], False)
    systematics['scale_e_down'] = ('ESCALE_DOWN' , '_'+options.syst_e_scale+'Down', 'wt', [], False)
...
if options.syst_tau_scale_0pi != '':
    systematics['scale_t_0pi_up'] = ('TSCALE0PI_UP' , '_'+options.syst_tau_scale_0pi+'Up', 'wt', ['QCD'], False)
    systematics['scale_t_0pi_down'] = ('TSCALE0PI_DOWN' , '_'+options.syst_tau_scale_0pi+'Down', 'wt', ['QCD'], False)
if options.syst_tau_scale_1pi != '':
    systematics['scale_t_1pi_up'] = ('TSCALE1PI_UP' , '_'+options.syst_tau_scale_1pi+'Up', 'wt', ['QCD'], False)
    systematics['scale_t_1pi_down'] = ('TSCALE1PI_DOWN' , '_'+options.syst_tau_scale_1pi+'Down', 'wt', ['QCD'], False)
...
if options.method in [17,18] and options.do_ff_systs and options.channel in ['et','mt','tt'] and options.era in ['cpsummer16','cpdecay16',"legacy16",'cpsummer17','smsummer16','mvadm2016']:
    processes = ['tt','w','qcd']
    dms = ['dm0', 'dm1']
    njets = ['njet0','njet1']
    for process in processes:
...
def GenerateFakeTaus(ana, add_name='', data=[], plot='',plot_unmodified='', wt='', sel='', cat_name='',get_os=True,ff_syst_weight=None):
    ...
    sub_wt=''
    if 'ff_sub_syst' in add_name and 'Up' in add_name: sub_wt='*1.1'
    if 'ff_sub_syst' in add_name and 'Down' in add_name: sub_wt='*0.9'
        ...
        if options.era in ['smsummer16','cpsummer16','cpdecay16',"legacy16",'cpsummer17','mvadm2016','cp18']:
          if ff_syst_weight is not None and 'ff_sub_syst' not in add_name: fake_factor_wt_string = ff_syst_weight+'_1'
            ...
            if options.analysis == 'cpprod': 
              fake_factor_wt_string = "wt_ff_dmbins_wjets_1"
            else: 
              fake_factor_wt_string = "wt_ff_mssm_wjets_1"
#              fake_factor_wt_string = "(((n_prebjets==0)*(jet_pt_2>=1.5*pt_2)*(0.244931*TMath::Landau(min(pt_2,599.),25.3803,8.03464)+0.0339215))+((n_prebjets==0)*(jet_pt_2<1.25*pt_2)*(((min(pt_2,599.)<140)*(74.4171*TMath::Landau(min(pt_2,599.),-165.923,11.7444)+3.25641e-07))+(0.172492*(min(pt_2,599.)>=140&&min(pt_2,599.)<200))+(0.272125*(min(pt_2,599.)>=200))))+((n_prebjets==0)*((jet_pt_2>=1.25*pt_2) && (jet_pt_2<1.5*pt_2))*(((min(pt_2,599.)<140)*(27.6762*TMath::Landau(min(pt_2,599.),-11.884,2.18228)+0.0768372))+(0.122975*(min(pt_2,599.)>=140))))+((n_prebjets>0)*(jet_pt_2>=1.5*pt_2)*(3.26931*TMath::Landau(min(pt_2,599.),-45.7731,6.88651)+0.030502))+((n_prebjets>0)*(jet_pt_2<1.25*pt_2)*(((min(pt_2,599.)<140)*(128.39*TMath::Landau(min(pt_2,599.),-177.43,8.91796)+1.11059e-11))+(0.158672*(min(pt_2,599.)>=140&&min(pt_2,599.)<200))+(0.187446*(min(pt_2,599.)>=200))))+((n_prebjets>0)*((jet_pt_2>=1.25*pt_2) && (jet_pt_2<1.5*pt_2))*(((min(pt_2,599.)<140)*(22.2849*TMath::Landau(min(pt_2,599.),-24.5922,3.04587)+0.0716253))+(0.0925934*(min(pt_2,599.)>=140&&min(pt_2,599.)<200))+(0.0952554*(min(pt_2,599.)>=200)))))"#*(((pt_1<=25&&(n_prebjets==0))*(1.435))+((pt_1>25&&(n_prebjets==0))*(1.16403+-0.00358314*pow(min(pt_1,160),1)))+((pt_1<=25&&(n_prebjets>0))*(1.38791))+((pt_1>25&&(n_prebjets>0))*(1.06894+0.000599205*pow(min(pt_1,200),1)+-3.98252e-05*pow(min(pt_1,200),2)+1.23692e-07*pow(min(pt_1,200),3))))"#*((((n_prebjets==0))*(1.04175+0.000275833*pow(min(met,140),1)+-2.25176e-05*pow(min(met,140),2)+7.14242e-08*pow(min(met,140),3)))+(((n_prebjets>0))*(0.970844+0.00257767*pow(min(met,200),1)+-3.98459e-05*pow(min(met,200),2)+1.17396e-07*pow(min(met,200),3))))"#*((((n_deepbjets>0 && mt_1<50))*(0.877036))+(((n_deepbjets==0 && mt_1<50))*(0.926825+0.000411168*pow(min(pt_1,200),1)))+(((n_deepbjets==0 && mt_1>50 && mt_1<70))*(1.00845+-0.000448315*pow(min(pt_1,200),1)))+(((n_deepbjets>0 && mt_1>50 && mt_1<70))*(0.981656))+(mt_1>70)))"
#
#              fake_factor_wt_string += "*((n_prebjets>0)*((9.64475e-02)*TMath::Erf((x+1.55816e+00)/2.54420e-01)+9.64317e-01) + (n_prebjets==0)*((2.79975e-01)*TMath::Erf((x+2.01373e+00)/8.32529e-01)+8.25687e-01))".replace("x","newmet*cos(newmet_dphi_2)/pt_2")
#              fake_factor_wt_string +="*((n_prebjets>0)*(1.11574-0.00224531*min(pt_1,200)) + (n_prebjets==0)*(1.20083-0.00376543*min(pt_1,100)))"

              #fake_factor_wt_string = '(((n_prebjets==0)*(jet_pt_2>=1.5*pt_2)*(4.28237*TMath::Landau(min(pt_2,599.),-31.3176,5.9833)+0.0243172))+((n_prebjets==0)*(jet_pt_2<1.25*pt_2)*(((min(pt_2,599.)<140)*(95.7998*TMath::Landau(min(pt_2,599.),-156.439,9.94146)+8.83542e-08))+(0.172607*(min(pt_2,599.)>=140&&min(pt_2,599.)<200))+(0.272306*(min(pt_2,599.)>=200))))+((n_prebjets==0)*((jet_pt_2>=1.25*pt_2) && (jet_pt_2<1.5*pt_2))*(((min(pt_2,599.)<140)*(10.9977*TMath::Landau(min(pt_2,599.),-10.0536,3.32525)+0.0764664))+(0.123059*(min(pt_2,599.)>=140))))+((n_prebjets>0)*(jet_pt_2>=1.5*pt_2)*(6.28094*TMath::Landau(min(pt_2,599.),9.40069,1.22798)+0.042003))+((n_prebjets>0)*(jet_pt_2<1.25*pt_2)*(((min(pt_2,599.)<140)*(105.313*TMath::Landau(min(pt_2,599.),-199.089,10.6973)+7.2051e-11))+(0.158641*(min(pt_2,599.)>=140&&min(pt_2,599.)<200))+(0.187409*(min(pt_2,599.)>=200))))+((n_prebjets>0)*((jet_pt_2>=1.25*pt_2) && (jet_pt_2<1.5*pt_2))*(((min(pt_2,599.)<140)*(58.542*TMath::Landau(min(pt_2,599.),-159.613,8.72111)+6.82588e-09))+(0.0926537*(min(pt_2,599.)>=140&&min(pt_2,599.)<200))+(0.0953174*(min(pt_2,599.)>=200)))))*((((n_prebjets==0))*(0.291178*TMath::Erf((max(min(newmet*cos(newmet_dphi_2)/pt_2,1.0),-3.5)--2.04946)/0.947901)+0.811864))+(((n_prebjets>0))*(0.0845921*TMath::Erf((max(min(newmet*cos(newmet_dphi_2)/pt_2,1.0),-3.5)--1.5259)/0.223841)+0.975683)))*((((n_prebjets==0)&&pt_1>=25)*(1.13548+-0.00286072*pow(min(pt_1,100),1))+((n_prebjets==0)&&pt_1<25)*(1.39906378221))+(((n_prebjets>0)&&pt_1>=25)*(1.11004+-0.00214286*pow(min(pt_1,200),1))+((n_prebjets>0)&&pt_1<25)*(1.37381972883)))'


#              if options.era=='cpsummer17':
#                fake_factor_wt_string = '((trg_singlemuon&&pt_1>25)*((n_jets==0)*((mva_dm_2==0&&ip_sig_2<1)*(1.082138*TMath::Landau(min(pt_2,140.),29.415497,15.507751)+0.045289)+(mva_dm_2==0&&ip_sig_2>=1)*(1.095232*TMath::Landau(min(pt_2,140.),28.733150,6.048563)+0.182775)+(mva_dm_2==1)*(0.250055*TMath::Landau(min(pt_2,140.),30.779041,4.368588)+0.182452)+(mva_dm_2==2)*(0.980577*TMath::Landau(min(pt_2,140.),92.395074,67.039495)+-0.051026)+(mva_dm_2==10)*(0.252194*TMath::Landau(min(pt_2,140.),47.647416,10.052655)+0.092086)+(mva_dm_2==11)*(0.054278+0.000111*min(pt_2,140.))) + (n_jets==1)*((mva_dm_2==0&&ip_sig_2<1)*(0.174600+-0.000489*min(pt_2,140.))+(mva_dm_2==0&&ip_sig_2>=1)*(0.973688*TMath::Landau(min(pt_2,140.),26.829827,3.045571)+0.197991)+(mva_dm_2==1)*(0.168757*TMath::Landau(min(pt_2,140.),39.681144,6.863146)+0.156294)+(mva_dm_2==2)*(1.201708*TMath::Landau(min(pt_2,140.),91.305097,68.790065)+-0.106601)+(mva_dm_2==10)*(25813.127411*TMath::Landau(min(pt_2,140.),1829.197891,7890.410268)+-4663.162873)+(mva_dm_2==11)*(0.145445*TMath::Landau(min(pt_2,140.),59.042272,20.504927)+0.030565)) + (n_jets>1)*((mva_dm_2==0&&ip_sig_2<1)*(0.523833*TMath::Landau(min(pt_2,140.),32.955492,2.235142)+0.130714)+(mva_dm_2==0&&ip_sig_2>=1)*(0.307755+-0.001141*min(pt_2,140.))+(mva_dm_2==1)*(62.249846*TMath::Landau(min(pt_2,140.),203.398320,490.850296)+-11.047284)+(mva_dm_2==2)*(1.896423*TMath::Landau(min(pt_2,140.),126.021709,113.557172)+-0.237262)+(mva_dm_2==10)*(2.737947*TMath::Landau(min(pt_2,140.),148.180381,144.375204)+-0.373127)+(mva_dm_2==11)*(0.301768*TMath::Landau(min(pt_2,140.),265.859964,221.406572)+0.000001))) + ((trg_singlemuon&&pt_1>25)==0)*((n_jets==0)*((mva_dm_2==0&&ip_sig_2<1)*(0.306789+-0.002156*min(min(pt_2,140.),80.))+(mva_dm_2==0&&ip_sig_2>=1)*(0.367306+0.002405*min(min(pt_2,140.),80.))+(mva_dm_2==1)*(0.261335+-0.000230*min(min(pt_2,140.),80.))+(mva_dm_2==2)*(0.087964+0.001340*min(min(pt_2,140.),80.))+(mva_dm_2==10)*(0.105357+0.001630*min(min(pt_2,140.),80.))+(mva_dm_2==11)*(-0.019499+0.001971*min(min(pt_2,140.),80.))) + (n_jets==1)*((mva_dm_2==0&&ip_sig_2<1)*(-0.214629+0.010504*min(min(pt_2,140.),80.))+(mva_dm_2==0&&ip_sig_2>=1)*(0.558432+-0.005462*min(min(pt_2,140.),80.))+(mva_dm_2==1)*(0.390775+-0.003478*min(min(pt_2,140.),80.))+(mva_dm_2==2)*(0.236525+-0.001701*min(min(pt_2,140.),80.))+(mva_dm_2==10)*(-0.059682+0.004323*min(min(pt_2,140.),80.))+(mva_dm_2==11)*(0.152114+-0.001669*min(min(pt_2,140.),80.))) + (n_jets>1)*((mva_dm_2==0&&ip_sig_2<1)*(0.361360+-0.000286*min(min(pt_2,140.),80.))+(mva_dm_2==0&&ip_sig_2>=1)*(0.488307)+(mva_dm_2==1)*(0.000134+0.004821*min(min(pt_2,140.),80.))+(mva_dm_2==2)*(0.104167+0.000504*min(min(pt_2,140.),80.))+(mva_dm_2==10)*(0.098968+0.000974*min(min(pt_2,140.),80.))+(mva_dm_2==11)*(0.032287))))'
#
#                fake_factor_wt_string+='*((n_jets==0)*(0.267308*TMath::Erf((max(min(newmet*cos(newmet_dphi_2)/pt_2,5),-5)--1.57651)/0.0134174)+0.93428) + (n_jets==1)*(3.5367*TMath::Erf((max(min(newmet*cos(newmet_dphi_2)/pt_2,5),-5)-12.3423)/12.2498)+4.10152) + (n_jets>=2)*(1.49895*TMath::Erf((max(min(newmet*cos(newmet_dphi_2)/pt_2,5),-5)-0.132968)/18.5461)+1.08379))*((pt_1>25)*((n_jets==0)*(0.455528+0.0411759*pow(min(pt_1,90.),1)+-0.000808969*pow(min(pt_1,90.),2)+4.30162e-06*pow(min(pt_1,90.),3)) + (n_jets==1)*(1.07798+-0.0022714*pow(min(pt_1,90.),1)) + (n_jets>=2)*(1.00685+-9.33302e-05*pow(min(pt_1,90.),1))) + (pt_1<25))'
#
#              if options.era=='legacy16':
#                fake_factor_wt_string = '((trg_singlemuon&&pt_1>23)*((n_jets==0)*((mva_dm_2==0&&ip_sig_2<1)*(0.531477*TMath::Landau(min(pt_2,140.),29.447499,8.423938)+0.138841)+(mva_dm_2==0&&ip_sig_2>=1)*(8.606710*TMath::Landau(min(pt_2,140.),-123.740483,27.601066)+0.000013)+(mva_dm_2==1)*(0.251975*TMath::Landau(min(pt_2,140.),35.306987,8.265922)+0.143757)+(mva_dm_2==2)*(2.646512*TMath::Landau(min(pt_2,140.),142.206925,179.491424)+-0.374229)+(mva_dm_2==10)*(1.061041*TMath::Landau(min(pt_2,140.),84.256543,52.169997)+-0.063227)+(mva_dm_2==11)*(0.093713*TMath::Landau(min(pt_2,140.),44.189862,10.786929)+0.037618)) + (n_jets==1)*((mva_dm_2==0&&ip_sig_2<1)*(6.514069*TMath::Landau(min(pt_2,140.),0.496816,1.883231)+0.135629)+(mva_dm_2==0&&ip_sig_2>=1)*(1.473751*TMath::Landau(min(pt_2,140.),26.531811,23.927555)+0.000007)+(mva_dm_2==1)*(0.245495*TMath::Landau(min(pt_2,140.),41.772683,2.503208)+0.147066)+(mva_dm_2==2)*(20.878487*TMath::Landau(min(pt_2,140.),6788.151620,4263.162470)+-1.817485)+(mva_dm_2==10)*(0.254198*TMath::Landau(min(pt_2,140.),71.084902,25.493234)+0.059259)+(mva_dm_2==11)*(1.119887*TMath::Landau(min(pt_2,140.),110.494293,100.345439)+-0.144669)) + (n_jets>1)*((mva_dm_2==0&&ip_sig_2<1)*(0.162310+-0.000650*min(pt_2,140.))+(mva_dm_2==0&&ip_sig_2>=1)*(0.705031*TMath::Landau(min(pt_2,140.),24.010723,1.138001)+0.135864)+(mva_dm_2==1)*(2.293832*TMath::Landau(min(pt_2,140.),149.147993,159.081747)+-0.260438)+(mva_dm_2==2)*(0.265943*TMath::Landau(min(pt_2,140.),105.392348,35.390877)+0.054298)+(mva_dm_2==10)*(0.426047*TMath::Landau(min(pt_2,140.),43.369940,16.583431)+0.012692)+(mva_dm_2==11)*(90.498422*TMath::Landau(min(pt_2,140.),302.277798,967.225333)+-16.300656))) + ((trg_singlemuon&&pt_1>23)==0)*((n_jets==0)*((mva_dm_2==0&&ip_sig_2<1)*(0.187839+0.001535*min(min(pt_2,140.),80.))+(mva_dm_2==0&&ip_sig_2>=1)*(0.549983+-0.005420*min(min(pt_2,140.),80.))+(mva_dm_2==1)*(0.248411+-0.000926*min(min(pt_2,140.),80.))+(mva_dm_2==2)*(0.063973+0.000755*min(min(pt_2,140.),80.))+(mva_dm_2==10)*(0.052822+0.001807*min(min(pt_2,140.),80.))+(mva_dm_2==11)*(0.065154+-0.000367*min(min(pt_2,140.),80.))) + (n_jets==1)*((mva_dm_2==0&&ip_sig_2<1)*(0.255425)+(mva_dm_2==0&&ip_sig_2>=1)*(0.317659)+(mva_dm_2==1)*(0.262228+-0.001425*min(min(pt_2,140.),80.))+(mva_dm_2==2)*(0.033024+0.000359*min(min(pt_2,140.),80.))+(mva_dm_2==10)*(0.056825+0.002631*min(min(pt_2,140.),80.))+(mva_dm_2==11)*(0.090117+-0.000708*min(min(pt_2,140.),80.))) + (n_jets>1)*((mva_dm_2==0&&ip_sig_2<1)*(0.205150+-0.000908*min(min(pt_2,140.),80.))+(mva_dm_2==0&&ip_sig_2>=1)*(0.264193)+(mva_dm_2==1)*(0.169507)+(mva_dm_2==2)*(0.144055+0.000081*min(min(pt_2,140.),80.))+(mva_dm_2==10)*(0.211971+-0.000450*min(min(pt_2,140.),80.))+(mva_dm_2==11)*(0.066762))))'
#
#                fake_factor_wt_string +='*((n_jets==0)*(0.435931*TMath::Erf((max(min(newmet*cos(newmet_dphi_2)/pt_2,5),-5)--1.59595)/0.617778)+0.922481) + (n_jets==1)*(69.2991*TMath::Erf((max(min(newmet*cos(newmet_dphi_2)/pt_2,5),-5)-15.1771)/7.51675)+70.0958) + (n_jets>=2)*(6.2911*TMath::Erf((max(min(newmet*cos(newmet_dphi_2)/pt_2,5),-5)-2.31677)/66.0137)+1.35217))*((pt_1>23)*((n_jets==0)*(0.325439+0.0480653*pow(min(pt_1,90.),1)+-0.000951283*pow(min(pt_1,90.),2)+5.30925e-06*pow(min(pt_1,90.),3)) + (n_jets==1)*(0.957977+-0.000135633*pow(min(pt_1,90.),1)) + (n_jets>=2)*(0.966316+0.000628719*pow(min(pt_1,90.),1))) + (pt_1<23))'

              #fake_factor_wt_string = '((trg_singlemuon&&pt_1>25)*((n_jets==0)*((mva_dm_2==0&&ip_sig_2<1)*(0.467659*TMath::Landau(min(pt_2,140.),30.694175,6.884141)+0.145984)+(mva_dm_2==0&&ip_sig_2>=1)*(13.952656*TMath::Landau(min(pt_2,140.),-74.648297,14.321614)+0.041666)+(mva_dm_2==1)*(0.184870*TMath::Landau(min(pt_2,140.),36.458005,5.411154)+0.189287)+(mva_dm_2==2)*(0.506730*TMath::Landau(min(pt_2,140.),69.768519,35.956814)+0.031816)+(mva_dm_2==10)*(0.457457*TMath::Landau(min(pt_2,140.),57.815624,20.466217)+0.056877)+(mva_dm_2==11)*(0.082608*TMath::Landau(min(pt_2,140.),47.475066,11.809907)+0.048189)) + (n_jets==1)*((mva_dm_2==0&&ip_sig_2<1)*(18.224666*TMath::Landau(min(pt_2,140.),-445.674000,74.163918)+-0.301597)+(mva_dm_2==0&&ip_sig_2>=1)*(0.935094*TMath::Landau(min(pt_2,140.),30.892621,13.564829)+0.108131)+(mva_dm_2==1)*(0.225285*TMath::Landau(min(pt_2,140.),36.692897,5.711167)+0.152521)+(mva_dm_2==2)*(4.447537*TMath::Landau(min(pt_2,140.),298.070868,361.405452)+-0.659933)+(mva_dm_2==10)*(0.246208*TMath::Landau(min(pt_2,140.),53.937906,11.950789)+0.079627)+(mva_dm_2==11)*(0.309853*TMath::Landau(min(pt_2,140.),90.079866,62.245171)+-0.000003)) + (n_jets>1)*((mva_dm_2==0&&ip_sig_2<1)*(2.881878*TMath::Landau(min(pt_2,140.),-92.057579,29.280293)+0.000013)+(mva_dm_2==0&&ip_sig_2>=1)*(26.088649*TMath::Landau(min(pt_2,140.),-3.645216,1.869592)+0.152451)+(mva_dm_2==1)*(0.280722*TMath::Landau(min(pt_2,140.),39.486442,5.863062)+0.134250)+(mva_dm_2==2)*(0.462701*TMath::Landau(min(pt_2,140.),82.190318,42.281716)+-0.000000)+(mva_dm_2==10)*(0.646343*TMath::Landau(min(pt_2,140.),120.321186,71.977940)+-0.000002)+(mva_dm_2==11)*(0.240475*TMath::Landau(min(pt_2,140.),1179.265209,5524.208060)+0.000914))) + ((trg_singlemuon&&pt_1>25)==0)*((n_jets==0)*((mva_dm_2==0&&ip_sig_2<1)*(0.441912+-0.002389*min(min(pt_2,140.),80.))+(mva_dm_2==0&&ip_sig_2>=1)*(0.474930+0.000996*min(min(pt_2,140.),80.))+(mva_dm_2==1)*(0.245527+0.002166*min(min(pt_2,140.),80.))+(mva_dm_2==2)*(0.182926+-0.000984*min(min(pt_2,140.),80.))+(mva_dm_2==10)*(0.043272+0.002266*min(min(pt_2,140.),80.))+(mva_dm_2==11)*(-0.025989+0.002380*min(min(pt_2,140.),80.))) + (n_jets==1)*((mva_dm_2==0&&ip_sig_2<1)*(0.152930+0.001110*min(min(pt_2,140.),80.))+(mva_dm_2==0&&ip_sig_2>=1)*(0.246260+0.007928*min(min(pt_2,140.),80.))+(mva_dm_2==1)*(0.320629+-0.000943*min(min(pt_2,140.),80.))+(mva_dm_2==2)*(0.103484)+(mva_dm_2==10)*(0.087308+0.001570*min(min(pt_2,140.),80.))+(mva_dm_2==11)*(-0.025459+0.001892*min(min(pt_2,140.),80.))) + (n_jets>1)*((mva_dm_2==0&&ip_sig_2<1)*(0.050833)+(mva_dm_2==0&&ip_sig_2>=1)*(0.205866)+(mva_dm_2==1)*(0.204361)+(mva_dm_2==2)*(0.148394)+(mva_dm_2==10)*(0.259275+-0.002195*min(min(pt_2,140.),80.))+(mva_dm_2==11)*(0.039061))))'
#              fake_factor_wt_string += '*((n_jets==0)*(-0.0286078 + 0.412258*max(met_dphi_1,1.5)) + (n_jets>0))'
              #fake_factor_wt_string += '*((n_jets==0)*(0.69622 -0.518898*(min(max(met*cos(met_dphi_1)/pt_1,-2.5),0)) -0.0491069*(min(max(met*cos(met_dphi_1)/pt_1,-2.5),0))*(min(max(met*cos(met_dphi_1)/pt_1,-2.5),0)) + 0.0813011*(min(max(met*cos(met_dphi_1)/pt_1,-2.5),0))*(min(max(met*cos(met_dphi_1)/pt_1,-2.5),0))*(min(max(met*cos(met_dphi_1)/pt_1,-2.5),0)) + 0.0143588*(min(max(met*cos(met_dphi_1)/pt_1,-2.5),0))*(min(max(met*cos(met_dphi_1)/pt_1,-2.5),0))*(min(max(met*cos(met_dphi_1)/pt_1,-2.5),0))*(min(max(met*cos(met_dphi_1)/pt_1,-2.5),0))) + (n_jets>0))'

              #fake_factor_wt_string += '*((n_jets==0)*(1.21001-0.48096*min(met*(1+cos(met_dphi_1))/pt_2,4) +0.032214*min(met*(1+cos(met_dphi_1))/pt_2,4)*min(met*(1+cos(met_dphi_1))/pt_2,4) +0.00777281*min(met*(1+cos(met_dphi_1))/pt_2,4)*min(met*(1+cos(met_dphi_1))/pt_2,4)*min(met*(1+cos(met_dphi_1))/pt_2,4)) + (n_jets>0))'

              #fake_factor_wt_string += '*((n_jets==0)*6.63029e+00*TMath::Landau(min(met*(1+min(cos(met_dphi_1),0))/pt_2,4),-2.53982e-02,5.87393e-01) + (n_jets>0))'

#'*((n_jets==0)*1.37144e+02*TMath::Landau(max(met*(1+min(cos(met_dphi_1),0)),45),-1.29902e+02,1.09652e+01) + (n_jets>0))'

              #fake_factor_wt_string += '*((n_jets==0)*(0.459004*TMath::Erf((newmet*cos(newmet_dphi_2)/pt_2+1.62305)/0.685425)+0.868812) + (n_jets>0))'
#              fake_factor_wt_string += '*((n_jets==0)*(0.459004*TMath::Erf((newmet*cos(newmet_dphi_2)/pt_2+1.62305)/0.685425)+0.868812) + (n_jets==1)*(0.226838*TMath::Erf((newmet*cos(newmet_dphi_2)/pt_2+1.54737)/1.96339)+0.900099) + (n_jets>1))'
#              fake_factor_wt_string += '*((n_jets==0)*(0.977645+0.010022*min(pt_1,90)-0.000269985*min(pt_1,90)*min(pt_1,90) + (1.5968e-06)*min(pt_1,90)*min(pt_1,90)*min(pt_1,90))+(n_jets==1)*(1.07375 -0.00127575*min(pt_2,90))+(n_jets>1))'
        ...
        f2 = GetSubtractNode(ana,'',plot,plot_unmodified,wt+sub_wt,sel,ff_cat,ff_cat_data,8,1.0,get_os,False)
    ...
    if options.channel == 'tt':
        ...
        if ff_syst_weight is not None and 'ff_sub_syst' not in add_name: 
...
#'*((rho_id_1>=0.5)*1.15+(rho_id_1<0.5)*0.82)*((rho_id_2>=0.5)*1.15+(rho_id_2<0.5)*0.82)'
        ...
        if options.channel in ['mt','et','tt']:
          # need to add back the other fake components when testing the QCD FF validations
          if 'ZLL' not in samples_to_skip:
          ...
          if 'QCD' not in samples_to_skip:
              GenerateQCD(ana, add_name+'_res', data_samples, plot, plot_unmodified, wt, sel, cat, cat_data, 12, qcd_os_ss_ratio, not options.do_ss,wshift)
    else:
