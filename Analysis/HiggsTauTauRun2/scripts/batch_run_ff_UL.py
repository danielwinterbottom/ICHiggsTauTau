...
if options.force_pol1: output += '_pol1'
...
if year == "all":
  year_list = ["2016_preVFP","2016_postVFP","2017","2018"]
else:
  ...
  for yr in year_list:
    ...
    if not workspace:
      for mc_shift in ["","Up","Down"]:
        ...
        if ch in ["et","mt"]:
          run_cmd_ff = "python scripts/ff_lt_UL.py --channel=%(ch)s --year=%(yr)s --output_folder='%(output)s/%(ch)s/%(yr)s/' --mc_shift=%(mc_shift)s" % vars()
        elif ch in ["tt"]:
          run_cmd_ff = "python scripts/ff_tt_UL.py --year=%(yr)s --output_folder='%(output)s/%(ch)s/%(yr)s/'" % vars()
        ...
        if not (ch in ["tt"] and mc_shift in ["Up","Down"]):
          ...
          if options.force_pol1:
            output_folder='%(output)s/%(ch)s/%(yr)s/' % vars()
      ...
      if ch in ["et","mt"]:
        run_cmd_ws = "python scripts/make_ff_workspace_UL_lt.py --channel=%(ch)s --year=%(yr)s --output='%(output)s/workspaces/' --input='%(output)s/%(ch)s/%(yr)s/'" % vars()
      elif ch in ["tt"]:
        run_cmd_ws = "python scripts/make_ff_workspace_UL_tt.py --year=%(yr)s --output='%(output)s/workspaces/' --input='%(output)s/%(ch)s/%(yr)s/'" % vars()

      job_file = "%(output)s/jobs/ff_ws_%(ch)s_%(yr)s.sh" % vars()
      CreateBatchJob(job_file,cmssw_base,[run_cmd_ws])
      if not dry_run:
        if ch == "mt" and yr == "2018": 
          SubmitBatchJob(job_file,time=600,memory=24,cores=1)
        else:
          SubmitBatchJob(job_file,time=180,memory=24,cores=1)
