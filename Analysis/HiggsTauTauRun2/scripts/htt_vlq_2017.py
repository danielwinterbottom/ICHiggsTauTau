...
# python scripts/htt_vlq_2017.py --bkg --data --embed --sm --vlq_matched --vlq_interference --jetmetuncerts --scales="default,scale_t_0pi,scale_t_1pi,scale_t_3prong,scale_t_3prong1pi0,scale_efake_0pi,scale_efake_1pi,scale_mufake_0pi,scale_mufake_1pi,scale_e,scale_bfake,scale_b" --submit='./scripts/submit_ic_batch_job.sh "hep.q -l h_rt=0:180:0 -l h_vmem=24G"' --parajobs
...
CONFIG='scripts/config_vlq_2017.json'
...
FILELIST='filelists/Sep18_2017_MC_102X'

signal_mc = [ ]
signal_vh = [ ] 
signal_mc_ww = [ ]

if os.path.isfile("./jobs/files_per_sample_2017.txt"):
  os.system("mv ./jobs/files_per_sample_2017.txt ./jobs/files_per_sample_2017-%(BACKUPNAME)s.txt"%vars())
...
#  DATAPREFIX = DATAFILELIST.split("/")[2]
  ...
  if options.calc_lumi:
    for sa in data_samples:
        JOB='%s_2017' % (sa)
        JSONPATCH= (r"'{\"job\":{\"filelist\":\"%(DATAFILELIST)s_%(sa)s.dat\",\"file_prefix\":\"root://gfe02.grid.hep.ph.ic.ac.uk:1097//store/user/dwinterb/%(DATAPREFIX)s/\",\"sequences\":{\"em\":[],\"et\":[],\"mt\":[],\"tt\":[]}}, \"sequence\":{\"output_name\":\"%(JOB)s\",\"is_data\":true,\"lumi_mask_only\":true}}' "%vars());
    ...
    for sa in data_samples:
        JOB='%s_2017' % (sa)
        JSONPATCH= (r"'{\"job\":{\"filelist\":\"%(DATAFILELIST)s_%(sa)s.dat\",\"file_prefix\":\"root://gfe02.grid.hep.ph.ic.ac.uk:1097//store/user/dwinterb/%(DATAPREFIX)s/\",\"sequences\":{\"em\":[],\"et\":[],\"mt\":[],\"tt\":[],\"zmm\":[],\"zee\":[]}}, \"sequence\":{\"output_name\":\"%(JOB)s\",\"is_data\":true}}' "%vars());
        nfiles = sum(1 for line in open('%(DATAFILELIST)s_%(sa)s.dat' % vars()))
        nperjob = 40
...
if options.proc_embed or options.proc_all:        
  ...
  for sa in embed_samples:
    job_num=0
    JOB='%s_2017' % (sa)
    JSONPATCH= (r"'{\"job\":{\"filelist\":\"%(EMBEDFILELIST)s_%(sa)s.dat\",\"file_prefix\":\"root://gfe02.grid.hep.ph.ic.ac.uk:1097//store/user/dwinterb/%(EMBEDPREFIX)s//\",\"sequences\":{\"em\":[],\"et\":[],\"mt\":[],\"tt\":[],\"zmm\":[],\"zee\":[]}}, \"sequence\":{\"output_name\":\"%(JOB)s\",\"is_embedded\":true,%(jetuncert_string)s}}' "%vars());
...
if options.mg_signal or options.proc_sm or options.proc_vlq_matched or options.proc_vlq_interference or options.proc_all:
  ...
  for sa in signal_mc:
    if 'VectorLQ' in sa and 'interference' in sa:
      PREFIX = 'Aug25_MC_102X_2017'
      SIG_FILELIST ="./filelists/Aug25_2017_MC_102X"
      user='guttley'
    ...
    for FLATJSONPATCH in flatjsons:
      FLATJSONPATCH = FLATJSONPATCH.replace('^scale_efake_0pi_hi^scale_efake_0pi_lo','').replace('^scale_efake_1pi_hi^scale_efake_1pi_lo','').replace('^scale_mufake_0pi_hi^scale_mufake_0pi_lo','').replace('^scale_mufake_1pi_hi^scale_mufake_1pi_lo','')
      FLATJSONPATCH = FLATJSONPATCH.replace('^met_uncl_hi^met_uncl_lo','')
      print '%(SIG_FILELIST)s_%(sa)s.dat' %vars()
