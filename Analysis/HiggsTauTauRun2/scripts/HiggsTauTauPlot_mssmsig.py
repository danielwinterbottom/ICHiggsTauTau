...
import argparse
import ConfigParser
import UserCode.ICHiggsTauTau.plotting_mssmsig as plotting
...
defaults = { 
    ...
    "w_ff_closure":False,"mssm_signal_mA_tanb":None
...
parser.add_argument("--mssm_signal_mA_tanb", dest="mssm_signal_mA_tanb", type=str,
    help="Will add signal from [mA,tan(b)] list")
...
if options.era in ["mssmsummer16","smsummer16",'cpsummer16','cpdecay16',"legacy16",'tauid2016','mvadm2016']:
    ...
    if options.analysis == 'mssmrun2': top_samples = ['TT']
...
#wt_ff_dmbins_qcd_syst_down
            ...
            if options.analysis in ['cpprod','cpdecay','mssmrun2']:
...
def RunPlotting(ana, cat='',cat_data='', sel='', add_name='', wt='wt', do_data=True, samples_to_skip=[], outfile='output.root',ff_syst_weight=None):
    ...
    if options.analysis in ['cpdecay','cpprod','mssmrun2']: zll_samples+=ewkz_samples
...
def InterpolatingWithBinLowEdge(hist,x):
  x1 = hist.GetBinLowEdge(hist.FindBin(x))
  x2 = hist.GetBinLowEdge(hist.FindBin(x)+1)
  y1 = hist.GetBinContent(hist.FindBin(x))
  y2 = hist.GetBinContent(hist.FindBin(x)+1)
  y = (((y2-y1)/(x2-x1))*(x-x1)) + y1
  return y



if options.mssm_signal_mA_tanb != None:
  mssm_signal_mA_tanb = options.mssm_signal_mA_tanb.split('[')[1].split(']')[0].split(',')
  # Set up a TH1 for each var bin with ggh and bbh mass bins
  var_bins_list = options.var.split('[')[1].split(']')[0].split(',')
  var_bins = array('f', map(float,var_bins_list))

  H_A_output = ROOT.TH1D('H_A_mA{}_tanb{}'.format(mssm_signal_mA_tanb[0],mssm_signal_mA_tanb[1]),"",len(var_bins)-1,var_bins)

  var_bins_hist = {}
  mass_list = {}
  mass_list['ggH_t'] = ggh_masses
  mass_list['ggH_b'] = ggh_masses
  mass_list['ggH_i'] = ggh_masses
  mass_list['ggA_t'] = ggh_masses
  mass_list['ggA_b'] = ggh_masses
  mass_list['ggA_i'] = ggh_masses
  if (samp == 'bbH' and not options.bbh_nlo_masses) or samp == 'bbH-LO':
    mass_list['bbH'] = bbh_masses
  elif 'bbH' in samp:
    mass_list['bbH'] = bbh_nlo_masses

  bbh_mass_bins = array('f', map(float,mass_list['bbH']))
  ggh_mass_bins = array('f', map(float,mass_list['ggH_t']))

  all_higgs = ['bbH','ggH_t','ggH_b','ggH_i','ggA_t','ggA_b','ggA_i']

  for i in options.var.split('[')[1].split(']')[0].split(','):
    for proc in all_higgs:
      var_bins_hist[var_name+'__'+str(i)+'__'+proc] = ROOT.TH1D(var_name+'__'+str(i)+'__'+proc,"",len(bbh_mass_bins)-1, bbh_mass_bins)

  # Get entries from outfile
  for key,val in var_bins_hist.items():
    bin_val = key.split('__')[-2]
    proc = key.split('__')[-1]
    directory = outfile.Get(nodename)
    histo = directory.Get(proc+mass)
    for i in range(1,val.GetNbinsX()+1):
      mass = mass_list[proc][i-1]
      directory = outfile.Get(nodename)
      histo = directory.Get(proc+mass)
      var_bins_hist[key].SetBinContent(i,histo.GetBinContent(histo.FindBin(float(bin_val))))

  bbH_output = ROOT.TH1D('bbH_mA{}_tanb{}'.format(mssm_signal_mA_tanb[0],mssm_signal_mA_tanb[1]),"",len(var_bins)-1,var_bins)
  bbA_output = ROOT.TH1D('bbA_mA{}_tanb{}'.format(mssm_signal_mA_tanb[0],mssm_signal_mA_tanb[1]),"",len(var_bins)-1,var_bins)
  ggH_t_output = ROOT.TH1D('ggH_t_mA{}_tanb{}'.format(mssm_signal_mA_tanb[0],mssm_signal_mA_tanb[1]),"",len(var_bins)-1,var_bins)
  ggA_t_output = ROOT.TH1D('ggA_t_mA{}_tanb{}'.format(mssm_signal_mA_tanb[0],mssm_signal_mA_tanb[1]),"",len(var_bins)-1,var_bins)
  ggH_b_output = ROOT.TH1D('ggH_b_mA{}_tanb{}'.format(mssm_signal_mA_tanb[0],mssm_signal_mA_tanb[1]),"",len(var_bins)-1,var_bins)
  ggA_b_output = ROOT.TH1D('ggA_b_mA{}_tanb{}'.format(mssm_signal_mA_tanb[0],mssm_signal_mA_tanb[1]),"",len(var_bins)-1,var_bins)
  ggH_i_output = ROOT.TH1D('ggH_i_mA{}_tanb{}'.format(mssm_signal_mA_tanb[0],mssm_signal_mA_tanb[1]),"",len(var_bins)-1,var_bins)
  ggA_i_output = ROOT.TH1D('ggA_i_mA{}_tanb{}'.format(mssm_signal_mA_tanb[0],mssm_signal_mA_tanb[1]),"",len(var_bins)-1,var_bins)
  
  # Opening JSON file 
  with open('scripts/mssm_mAtanB.json') as json_file: 
    mssm_mAtanb = json.load(json_file) 
  H_mass = mssm_mAtanb['mA{}_tanb{}'.format(mssm_signal_mA_tanb[0],mssm_signal_mA_tanb[1])]['mH'] 
  A_mass = mssm_mAtanb['mA{}_tanb{}'.format(mssm_signal_mA_tanb[0],mssm_signal_mA_tanb[1])]['mA']
  bbH_xs = mssm_mAtanb['mA{}_tanb{}'.format(mssm_signal_mA_tanb[0],mssm_signal_mA_tanb[1])]['xs_bbH']
  ggH_xs = mssm_mAtanb['mA{}_tanb{}'.format(mssm_signal_mA_tanb[0],mssm_signal_mA_tanb[1])]['xs_ggH'] 
  bbA_xs = mssm_mAtanb['mA{}_tanb{}'.format(mssm_signal_mA_tanb[0],mssm_signal_mA_tanb[1])]['xs_bbA']
  ggA_xs = mssm_mAtanb['mA{}_tanb{}'.format(mssm_signal_mA_tanb[0],mssm_signal_mA_tanb[1])]['xs_ggA']
  HTT_br = mssm_mAtanb['mA{}_tanb{}'.format(mssm_signal_mA_tanb[0],mssm_signal_mA_tanb[1])]['br_Htt'] 
  ATT_br = mssm_mAtanb['mA{}_tanb{}'.format(mssm_signal_mA_tanb[0],mssm_signal_mA_tanb[1])]['br_Att']
  gt_H = mssm_mAtanb['mA{}_tanb{}'.format(mssm_signal_mA_tanb[0],mssm_signal_mA_tanb[1])]['gt_H']
  gt_A = mssm_mAtanb['mA{}_tanb{}'.format(mssm_signal_mA_tanb[0],mssm_signal_mA_tanb[1])]['gt_A']
  gb_H = mssm_mAtanb['mA{}_tanb{}'.format(mssm_signal_mA_tanb[0],mssm_signal_mA_tanb[1])]['gb_H']
  gb_A = mssm_mAtanb['mA{}_tanb{}'.format(mssm_signal_mA_tanb[0],mssm_signal_mA_tanb[1])]['gb_A']


  # Interpolate between bins
  for i in range(1,bbH_output.GetNbinsX()+1):
    bbH_output.SetBinContent(i,InterpolatingWithBinLowEdge(var_bins_hist[var_name+'__'+str(var_bins_list[i-1])+'__bbH'],H_mass))
    bbA_output.SetBinContent(i,InterpolatingWithBinLowEdge(var_bins_hist[var_name+'__'+str(var_bins_list[i-1])+'__bbH'],A_mass))
    ggH_t_output.SetBinContent(i,InterpolatingWithBinLowEdge(var_bins_hist[var_name+'__'+str(var_bins_list[i-1])+'__ggH_t'],H_mass))
    ggA_t_output.SetBinContent(i,InterpolatingWithBinLowEdge(var_bins_hist[var_name+'__'+str(var_bins_list[i-1])+'__ggA_t'],A_mass))
    ggH_b_output.SetBinContent(i,InterpolatingWithBinLowEdge(var_bins_hist[var_name+'__'+str(var_bins_list[i-1])+'__ggH_b'],H_mass))
    ggA_b_output.SetBinContent(i,InterpolatingWithBinLowEdge(var_bins_hist[var_name+'__'+str(var_bins_list[i-1])+'__ggA_b'],A_mass))
    ggH_i_output.SetBinContent(i,InterpolatingWithBinLowEdge(var_bins_hist[var_name+'__'+str(var_bins_list[i-1])+'__ggH_i'],H_mass))
    ggA_i_output.SetBinContent(i,InterpolatingWithBinLowEdge(var_bins_hist[var_name+'__'+str(var_bins_list[i-1])+'__ggA_i'],A_mass))

  bbH_output.Scale(bbH_xs*HTT_br)
  bbA_output.Scale(bbA_xs*ATT_br)
  ggH_t_output.Scale(ggH_xs*HTT_br*gt_H*gt_H)
  ggA_t_output.Scale(ggA_xs*ATT_br*gt_A*gt_A)
  ggH_b_output.Scale(ggH_xs*HTT_br*gb_H*gb_H)
  ggA_b_output.Scale(ggA_xs*ATT_br*gb_A*gb_A)
  ggH_i_output.Scale(ggH_xs*HTT_br*gt_H*gb_H)
  ggA_i_output.Scale(ggA_xs*ATT_br*gt_A*gb_A)

  H_A_output.Add(bbH_output)
  H_A_output.Add(bbA_output)
  H_A_output.Add(ggH_t_output)
  H_A_output.Add(ggA_t_output)
  H_A_output.Add(ggH_b_output)
  H_A_output.Add(ggA_b_output)
  H_A_output.Add(ggH_i_output)
  H_A_output.Add(ggA_i_output)
  
  for i in range(1,H_A_output.GetNbinsX()+1):
    if H_A_output.GetBinContent(i) <= 0:
      H_A_output.SetBinContent(i,0)
