...
FILELIST='filelists/May21_2016_MC_102X'
...
if options.proc_data or options.proc_all or options.calc_lumi:  
  ...
  DATAFILELIST="./filelists/May21_2016_Data_102X"

  for sa in data_samples:
      JOB='%s_2016' % (sa)
      JSONPATCH= (r"'{\"job\":{\"filelist\":\"%(DATAFILELIST)s_%(sa)s.dat\",\"file_prefix\":\"root://gfe02.grid.hep.ph.ic.ac.uk:1097//store/user/guttley/May21_Data_102X_2016/\",\"sequences\":{\"em\":[],\"et\":[],\"mt\":[],\"tt\":[],\"zmm\":[],\"zee\":[]}}, \"sequence\":{\"output_name\":\"%(JOB)s\",\"is_data\":true}}' "%vars());
      nfiles = sum(1 for line in open('%(DATAFILELIST)s_%(sa)s.dat' % vars()))

      # if 'TauB' in sa:
      #   JSONPATCH= (r"'{\"job\":{\"filelist\":\"./filelists/Oct2_Data_102X_%(sa)s.dat\",\"file_prefix\":\"root://gfe02.grid.hep.ph.ic.ac.uk:1097//store/user/guttley/Oct2_Data_102X_2016/\",\"sequences\":{\"em\":[],\"et\":[],\"mt\":[],\"tt\":[],\"zmm\":[],\"zee\":[]}}, \"sequence\":{\"output_name\":\"%(JOB)s\",\"is_data\":true}}' "%vars());
...
if options.proc_embed or options.proc_all:        
  ...
  EMBEDFILELISTZMM="./filelists/May21_2016_MC_102X"

  EMBEDFILELIST="./filelists/May21_2016_MC_102X"
  
  for sa in embed_samples:
    job_num=0  
    JOB='%s_2016' % (sa)
    JSONPATCH= (r"'{\"job\":{\"filelist\":\"%(EMBEDFILELIST)s_%(sa)s.dat\",\"file_prefix\":\"root://gfe02.grid.hep.ph.ic.ac.uk:1097//store/user/guttley/May21_MC_102X_2016/\",\"sequences\":{\"em\":[],\"et\":[],\"mt\":[],\"tt\":[],\"zmm\":[],\"zee\":[]}}, \"sequence\":{\"output_name\":\"%(JOB)s\",\"is_embedded\":true}}' "%vars());
    if 'EmbeddingMuMu' in sa:
      JSONPATCH= (r"'{\"job\":{\"filelist\":\"%(EMBEDFILELISTZMM)s_%(sa)s.dat\",\"file_prefix\":\"root://gfe02.grid.hep.ph.ic.ac.uk:1097//store/user/guttley/May21_MC_102X_2016/\",\"sequences\":{\"em\":[],\"et\":[],\"mt\":[],\"tt\":[],\"zmm\":[],\"zee\":[]}}, \"sequence\":{\"output_name\":\"%(JOB)s\",\"is_embedded\":true}}' "%vars());
...
if options.proc_bkg or options.proc_all:
  ...
  for sa in central_samples:
      JOB='%s_2016' % (sa)
      BKG_FILELIST = FILELIST
      user = 'guttley'
      PREFIX='May21_MC_102X_2016'
      if sa.startswith('TTTo'):
        BKG_FILELIST = './filelists/May21_2016_MC_102X'
        user='guttley'
        PREFIX='May21_MC_102X_2016'
      ...
      for FLATJSONPATCH in flatjsons: 
        nperjob = 20
        if sa.startswith('TT'): nperjob = 10
        ...
        nfiles = sum(1 for line in open('%(BKG_FILELIST)s_%(sa)s.dat' % vars()))
        for i in range (0,int(math.ceil(float(nfiles)/float(nperjob)))) :
          os.system('%(JOBWRAPPER)s "./bin/HTT --cfg=%(CONFIG)s --json=%(JSONPATCH)s --flatjson=%(FLATJSONPATCH)s --offset=%(i)d --nlines=%(nperjob)d &> jobs/%(JOB)s-%(job_num)d.log" jobs/%(JOB)s-%(job_num)s.sh' %vars())
          if not parajobs and not options.condor:
              os.system('%(JOBSUBMIT)s jobs/%(JOB)s-%(job_num)d.sh' % vars())
...
if options.proc_sm or options.proc_all:
  SIG_FILELIST = './filelists/May21_2016_MC_102X'
  user = "guttley"
  for sa in signal_mc:
    JOB='%s_2016' % (sa)
    PREFIX = 'May21_MC_102X_2016'
