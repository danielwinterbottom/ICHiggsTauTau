...
for sa in sample_list:
  ...
  for ch in channel:
    for jsdir in subdirs:
      sdir = jsdir[0]
      infiles=jsdir[1]
      if os.path.isfile('%(outputf)s/%(sdir)s/%(sa)s_%(ch)s_2017_0.root'%vars()):
          if not batch:  
            print "Hadding in subdir %(sdir)s"%vars()
            print "Hadding %(sa)s_%(ch)s in %(sdir)s"%vars()
            #os.system('cp %(outputf)s/%(sdir)s/%(sa)s_%(ch)s_2017.root %(outputf)s/%(sdir)s/%(sa)s_%(ch)s_old_2017.root'% vars()) 
            os.system('hadd -f %(outputf)s/%(sdir)s/%(sa)s_%(ch)s_2017.root %(outputf)s/%(sdir)s/%(sa)s_%(ch)s_2017_*.root &> ./haddout.txt'% vars()) 
            ...
            if warning or 'Error' in filetext:
              ...
              to_remove.append('rm %(outputf)s/%(sdir)s/%(sa)s_%(ch)s_2017_*.root' %vars())
          else:
            haddout='haddout_%s_%s_%s.txt' % (sa,ch,sdir)  
            hadd_dirs.append((haddout, 'rm %(outputf)s/%(sdir)s/%(sa)s_%(ch)s_2017_*.root' %vars())) 
            command+="echo \"Hadding %(sa)s_%(ch)s in %(sdir)s\"\necho \"Hadding %(sa)s_%(ch)s\"\nhadd -f %(outputf)s/%(sdir)s/%(sa)s_%(ch)s_2017.root %(outputf)s/%(sdir)s/%(sa)s_%(ch)s_2017_*.root &> ./%(haddout)s\nsed -i '/Warning in <TInterpreter::ReadRootmapFile>/d' ./%(haddout)s\n" % vars()     
