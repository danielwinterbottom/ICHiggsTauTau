...
for wp in wps:
  ...
  for dm in dm_bins:
    for njet in ['0','1','2','0_crosstrg','1_crosstrg','2_crosstrg']:
      ...
      hist_w_nom = GetFromTFile(loc+'fakefactor_fits_et_%(wp)s_2017.root:%(dm)s_njets%(njet)s_pt_2_ff_wjets_uncert' % vars())
      hist_qcd_nom = GetFromTFile(loc+'fakefactor_fits_et_%(wp)s_2017.root:%(dm)s_njets%(njet)s_pt_2_ff_qcd_uncert' % vars())
...
# systematic shifts for ttbar
  ...
  for dmtype in ['mvadm','mvadm_nosig','dm']:
    ...
    for njet in ['0','1','2']:
      func = GetFromTFile(loc+'fakefactor_fits_et_%(wp)s_2017.root:%(dmtype)s_WpT_corr_njet%(njet)s_wjets_mc_fit' % vars())
      corr_str = str(func.GetExpFormula('p')).replace('x','@0').replace(',false','')
