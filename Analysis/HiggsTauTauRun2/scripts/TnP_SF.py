...
nom_fname = "{}/{}_nom.root".format(input_folder,file_name)
if not os.path.exists(nom_fname):
  print "{} does not exist, exiting".format(nom_fname)
  exit()
f_nom = ROOT.TFile.Open(nom_fname)
if options.run_systs:
   f_sig = ROOT.TFile.Open("{}/{}_sig.root".format(input_folder,file_name))
   f_bkg = ROOT.TFile.Open("{}/{}_bkg.root".format(input_folder,file_name))
   f_tag = ROOT.TFile.Open("{}/{}_tightTag.root".format(input_folder,file_name))

output_folder = "{}/{}/correction/".format(input_folder,channel)
  ...
  for xbin in range(1, h_SF.GetNbinsX()+1):
      for ybin in range(1, h_SF.GetNbinsY()+1):
          ...
          if name == 'trg' and data_y_val<0.1:
            ...
            if embed: 
              content_nom_EMB = 0 if num_only else h_nom_EMB.GetBinContent(xbin,ybin)
            if options.run_systs:
              ...
              if embed and not num_only:
                content_sig_EMB = 0 if num_only else h_sig_EMB.GetBinContent(xbin, ybin)
                content_bkg_EMB = 0 if num_only else h_bkg_EMB.GetBinContent(xbin,ybin)
                content_tag_EMB = 0 if num_only else h_tag_EMB.GetBinContent(xbin,ybin) 
