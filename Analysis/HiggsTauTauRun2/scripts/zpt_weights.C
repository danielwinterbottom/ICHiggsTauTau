...
int main(){
std::string mvis[] =  {"50","60","70","80","90","100","120","140","160","180","200","300","400","600","800","1000"};

Float_t ybins[]={0,10,20,30,40,60,80,100,120,160,200,280,320,400,600}; //pt   
Float_t xbins[]={0,50,60,70,80,90,100,120,140,160,180,200,300,400,600,800,1000}; //mvis  

TH2D *zptmass_histo = new TH2D("zptmass_histo","zptmass_histo",16,xbins,14,ybins);
TH2D *zptmass_histo_err = new TH2D("zptmass_histo_err","zptmass_histo_err",16,xbins,14,ybins);
TH2D *data_sub_2D = new TH2D("data_sub_2D","data_sub_2D",16,xbins,14,ybins);
TH2D *ZLL_2D = new TH2D("ZLL_2D","ZLL_2D",16,xbins,14,ybins);

const char* years[4] = { "2016_preVFP", "2016_postVFP", "2017", "2018" };
for (int j = 0; j < 4; i++){
	
	for(int i=0; i<((sizeof(mvis)/sizeof(*mvis))-1); i++) {
		if(years[j] == "2016_preVFP" || years[j] == "2016_postVFP") {
			TFile *file1 = TFile::Open(("input/plots_LO/"+years[j]+"/datacard_pt_tt_inclusive_zmm_2016_mvis"+mvis[i]+"to"+mvis[i+1]+".root").c_str());
			std::cout << "Reading file: " << "datacard_pt_tt_inclusive_zmm_2016_mvis"+mvis[i]+"to"+mvis[i+1]+".root" << std::endl;
		} else if(years[j] == "2017"){
			TFile *file1 = TFile::Open(("input/plots_LO/"+years[j]+"/datacard_pt_tt_inclusive_zmm_2017_mvis"+mvis[i]+"to"+mvis[i+1]+".root").c_str());
			std::cout << "Reading file: " << "datacard_pt_tt_inclusive_zmm_2017_mvis"+mvis[i]+"to"+mvis[i+1]+".root" << std::endl;
		} else {
			TFile *file1 = TFile::Open(("input/plots_LO/"+years[j]+"/datacard_pt_tt_inclusive_zmm_2018_mvis"+mvis[i]+"to"+mvis[i+1]+".root").c_str());
			std::cout << "Reading file: " << "datacard_pt_tt_inclusive_zmm_2018_mvis"+mvis[i]+"to"+mvis[i+1]+".root" << std::endl;
		}
