...
if year == '2018':
  ...
  input_folder = '/vols/cms/gu18/Offline/output/MSSM/mssm_2018_v4/'
...
def FitFakeFactors(h,usePol1=False,polOnly=None,use_erf=False):
  ...
  if h.GetBinContent(h.GetNbinsX()) > 0 and h.GetBinError(h.GetNbinsX())/h.GetBinContent(h.GetNbinsX()) <0.5:
    if 'wjets' in h.GetName() and channel == 'mt' and h.GetBinContent(h.GetNbinsX()-1) > 0 and h.GetBinError(h.GetNbinsX()-1)/h.GetBinContent(h.GetNbinsX()-1)<0.5 and not ('wjets_mc' in h.GetName() and 'pt_low_1jet' in h.GetName() and year == '2016') and h.GetBinContent(h.GetNbinsX()-1) > h.GetBinContent(h.GetNbinsX()-2):
      if use_erf:
  ...
  if polOnly is None:
    ...
    if int(status) != 0  and not use_erf:
      ...
      if use_erf:
        f2.SetParameter(0,f1.GetParameter(0)); f2.SetParameter(1,f1.GetParameter(1)); f2.SetParameter(2,f1.GetParameter(2)); f2.SetParLimits(2,0.001,100) #f2.SetParLimits(0,0,100)
      else:
...
wjets_cw_cap = {'0jet':
                           {
                            'mt':{'2016':[-2.3,1],'2017':[-2.5,1],'2018':[-2.3,1]},
                            'et':{'2016':[-2.5,1],'2017':[-2.2,1],'2018':[-2.3,1]},
                           },
                    '1jet':
                           {
                            'mt':{'2016':[-2.3,1],'2017':[-2.2,1],'2018':[-2.6,1]},
                            'et':{'2016':[-2.2,1],'2017':[-2.2,1],'2018':[-2.2,1]},
                           },
...
for njets_name, njets_cut in njets_bins.items():
  if "inclusive" not in njets_name:
    ...
    PlotFakeFactorCorrection(wjets_data, wjets_data_uncert, wjets_data.GetName(), output_folder, wp, channel, year, wjets_cw_cap[njets_name][channel][year][0], wjets_cw_cap[njets_name][channel][year][1], x_title='c_{W}',logx=False)
...
for njets_name, njets_cut in njets_bins.items():
  if "inclusive" not in njets_name:
    ...
    PlotFakeFactorCorrection(wjets_mc_data, wjets_mc_data_uncert, wjets_mc_data.GetName(), output_folder, wp, channel, year, wjets_cw_cap[njets_name][channel][year][0], wjets_cw_cap[njets_name][channel][year][1], x_title='c_{W}',logx=False)
...
for add_name, corr_cut in categories.items():
  ...
  wjets_mc_data_fit.Write()
  wjets_mc_data_uncert.Write()
  PlotFakeFactorCorrection(wjets_mc_data, wjets_mc_data_uncert, wjets_mc_data.GetName(), output_folder, wp, channel, year, 0, 200, x_title='p_{T}^{%(lep_name)s} (GeV)' % vars(),logx=True,first_bin=True)
...
PlotFakeFactorCorrection(ttbar_mc_data, ttbar_mc_data_uncert, ttbar_mc_data.GetName(), output_folder, wp, channel, year, low_cap, 1, x_title='c_{W}',logx=False)
...
for add_name, add_cut in mt_1_regions.items():
  ...
  ttbar_mc_data_fit.Write()
  ttbar_mc_data_uncert.Write()
  PlotFakeFactorCorrection(ttbar_mc_data, ttbar_mc_data_uncert, ttbar_mc_data.GetName(), output_folder, wp, channel, year, 0, 200, x_title='p_{T}^{%(lep_name)s} (GeV)' % vars(),logx=True,first_bin=True)
