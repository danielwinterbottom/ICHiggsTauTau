...
for file_ in files:
    for path in glob.glob(sys.argv[1]):
        ...
        if year == "2016" and split == "pre":  new_file = ROOT.TFile.Open(os.path.join(path, "pileupDistribution/pileup_{}_preVFP_{}.root".format(year,file_)), "RECREATE")
        elif year == "2016" and split == "post": new_file = ROOT.TFile.Open(os.path.join(path, "pileupDistribution/pileup_{}_postVFP_{}.root".format(year,file_)), "RECREATE")
