...
parser.add_option("--cfg", dest="config", type='string', default='scripts/plot_cpprod_2018_v2.cfg',
...
if SCHEME == 'cpprod_newbins':
  ...
  #VAR_DIJET   =  'svfit_mass,sjdphi[50,80,100,115,130,150,200],(12,-3.2,3.2)'
  #VAR_DIJET   =  'svfit_mass,sjdphi[50,80,100,110,120,140,160,200],(12,-3.2,3.2)'
  #VAR_DIJET_MT   =  'svfit_mass,sjdphi[50,80,100,110,120,140,200],(12,-3.2,3.2)'
  #VAR_DIJET_TT   =  'svfit_mass,sjdphi[50,80,100,110,130,150,200],(12,-3.2,3.2)'
  #VAR_TT_LO_HI='svfit_mass,sjdphi[50,80,100,115,150,200],(12,-3.2,3.2)'
  VAR_DIJET   =  'svfit_mass,sjdphi[50,80,90,100,110,120,130,140,150,160,200]'
  VAR_DIJET_1 =  'svfit_mass,sjdphi[50,80,100,115,130,200],(12,-3.2,3.2)'
  VAR_DIJET_2 =  'svfit_mass,sjdphi[50,80,100,150,200],(12,-3.2,3.2)'
...
#        if ch in ['et'] and SCHEME != 'sync': extra+=' --add_wt=\"wt_btag*trigweight_1\" '
        if ch in ['et','mt','tt'] and cat_num in ['17','18'] and SCHEME != 'sync': extra+='  '

        extra_jes = options.extra + ' ' + extra_global + ' ' + jes_systematics + ' ' + opts + ' --no_default '

        if not options.hadd:
            if not options.batch:
                print 'python $CMSSW_BASE/src/UserCode/ICHiggsTauTau/Analysis/HiggsTauTauRun2/scripts/HiggsTauTauPlot.py --cfg=%(CFG)s --channel=%(ch)s --method=%(cat_num)s --cat=%(cat_str)s --year=%(YEAR)s --outputfolder=%(output_folder)s/ --datacard=%(dc)s --paramfile=%(PARAMS)s --folder=%(FOLDER)s %(BLIND)s --var="%(var)s" %(extra)s ' % vars()
