  ...
  for year in years:
    ...
    for ch in channels:
      ...
      for cat in categories:
        ...
        if control and "control" in cat: bins = BINS_CONTROL

        if syst:   
          add_cond += extra_channel[ch]

        run_cmd = 'python %(cmssw_base)s/src/UserCode/ICHiggsTauTau/Analysis/HiggsTauTauRun2/scripts/HiggsTauTauPlot.py --cfg=%(CFG)s --channel=%(ch)s --method=%(method)s --cat=%(cat)s --year=%(YEAR)s --outputfolder=%(output_folder)s/ --datacard=%(cat)s --paramfile=%(PARAMS)s --folder=%(FOLDER)s --var="%(var)s%(bins)s" --embedding --add_sm_background=125 --doMSSMReWeighting --wp=%(wp)s --no_plot %(add_cond)s' % vars()
