  ...
  if polOnly is None:
    ...
    if useExp:
      ...
      for i in range(1,h.GetNbinsX()+1):
        ...
        if bin_center < float(pt_min) or bin_center > float(pt_max)+20:
...
# mt plots
baseline_bothiso = 'iso_1<0.15 && deepTauVsJets_%(wp)s_2>0.5 && deepTauVsEle_vvloose_2>0.5 && deepTauVsMu_tight_2>0.5 && leptonveto==0 && ((trg_mutaucross&&pt_1<25)||(trg_singlemuon&&pt_1>25)) && wt<2 && jet_pt_2>0' % vars()
baseline_aiso1 = 'iso_1<0.15 && deepTauVsJets_%(wp)s_2<0.5 && deepTauVsJets_vvvloose_2>0.5 && deepTauVsEle_vvloose_2>0.5 && deepTauVsMu_tight_2>0.5 && leptonveto==0 && ((trg_mutaucross&&pt_1<25)||(trg_singlemuon&&pt_1>25)) && wt<2 && jet_pt_2>0' % vars()
...
baseline_aiso2_iso = 'iso_1<0.5&&iso_1>0.25 && deepTauVsJets_%(wp)s_2>0.5 && deepTauVsEle_vvloose_2>0.5 && deepTauVsMu_tight_2>0.5 && leptonveto==0 && ((trg_mutaucross&&pt_1<25)||(trg_singlemuon&&pt_1>25)) && wt<2 && jet_pt_2>0' % vars()
baseline_aiso2_aiso1 = 'iso_1<0.5&&iso_1>0.25 && deepTauVsJets_%(wp)s_2<0.5 && deepTauVsJets_vvvloose_2>0.5 && deepTauVsEle_vvloose_2>0.5 && deepTauVsMu_tight_2>0.5 && leptonveto==0 && ((trg_mutaucross&&pt_1<25)||(trg_singlemuon&&pt_1>25)) && wt<2 && jet_pt_2>0' % vars()
...
for n_bjets_name,n_bjets_cut in n_bjets_bins.items():
  ...
  name = 'mt_fracs_%(n_bjets_name)s' % vars()
...
#cut = ''
#var = 'n_bjets[1,9999]'
#(qcd_data, wjets_data, wjets_mc_data, ttbar_data) = DrawHists(var, '('+baseline_bothiso+')*('+cut+')', 'n_bjets1_closure' % vars(),input_folder,file_ext,False,doQCD=False,doW=True,doMC=False)
#(qcd_pred, wjets_pred, wjets_mc_pred, ttbar_pred) = DrawHists(var, '('+baseline_aiso1+')*('+cut+')', 'n_bjets1_closure_pred' % vars(),input_folder,file_ext,False,add_wt='%(ff_weight_cond)s' % vars() ,doQCD=False,doW=True,doMC=False)
   
#wjets_data.Divide(wjets_pred)
#wjets_n_bjets1_corr_fit, wjets_n_bjets1_corr_uncert = FitCorrection(wjets_data, func='pol0')
#wjets_data.Write()
#wjets_n_bjets1_corr_fit.Write()
#wjets_n_bjets1_corr_uncert.Write()
