...
namespace ic {
  ...
  L1TFilter::L1TFilter(std::string const& name, std::string channel, fwlite::TFileService *fs, struct L1Cuts l1Cuts, std::string L1MuTag, std::string subDirName) : ModuleBase(name) {
    ...
    h_jet1Eta = subDir.make<TH1D>("h_jet1Eta","h_jet1Eta",100, -5,5); 
    h_jet2Eta = subDir.make<TH1D>("h_jet2Eta","h_jet2Eta",100, -5,5); 
  ...
  int L1TFilter::Execute(TreeEvent *event) {
      ...
      if(n_l1jets_>1){
         ...
         for(unsigned i=0; i< n_l1jets_; i++){
             ...
             for(unsigned j=i+1; j< n_l1jets_; j++){
                 ...
                 if(Mjj >= MjjCut && DeltaEta >= DeltaEtaCut && (l1jets[i]->vector().Pt()+l1jets[j]->vector().Pt())/2 >= Jet1PtCut && (std::fabs(l1jets[i]->vector().Rapidity()) >= EtaCut || std::fabs(l1jets[j]->vector().Rapidity()) >= EtaCut) && (l1jets[i]->vector()+l1jets[j]->vector()).Pt() >= Jet2PtCut) dontFilter = true; 
                 
                 int count = 0;
                 if(dontFilter && count < 1){
                     h_jetsMjj->Fill(Mjj);
                     ...
                     h_jet1Eta->Fill(l1jets[0]->vector().Rapidity());
                     h_jet2Eta->Fill(l1jets[1]->vector().Rapidity());
                     count++;
                 }
             }
     ...
     //1.06382e+09
     if(!Filter) h_L1Filter_Efficiency->Fill(eventInfo->event());
