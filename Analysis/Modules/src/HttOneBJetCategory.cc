...
namespace ic {
  ...
  HttOneBJetCategory::HttOneBJetCategory(std::string const& name, std::string const& jets_label) : ModuleBase(name), channel_(channel::et) {
    ...
    onebjet_plots_.resize(4);
    onebjet_yields_.resize(4);
    onebjet_loose_plots_.resize(4);
    onebjet_loose_yields_.resize(4);
  ...
  HttOneBJetCategory::~HttOneBJetCategory() {
    ;
  }

  int HttOneBJetCategory::PreAnalysis() {
    std::cout << "----------------------------------------" << std::endl;
    std::cout << "PreAnalysis Info for HTT 1 b-Jet Category" << std::endl;
    std::cout << "----------------------------------------" << std::endl;
    std::cout << "HighPt Mode: [" << high_pt_ << "] Jets: [" << jets_label_ << "]" << std::endl;
    ...
    std::string pt_cat_string = high_pt_ ? "onebjet_highpt" : "onebjet_lowpt";
    onebjet_plots_[0] = new HttPlots(fs_->mkdir(pt_cat_string+"_os_sel"));
    onebjet_plots_[1] = new HttPlots(fs_->mkdir(pt_cat_string+"_ss_sel"));
    onebjet_plots_[2] = new HttPlots(fs_->mkdir(pt_cat_string+"_os_con"));
    onebjet_plots_[3] = new HttPlots(fs_->mkdir(pt_cat_string+"_ss_con"));

    onebjet_loose_plots_[0] = new HttPlots(fs_->mkdir(pt_cat_string+"_loose_os_sel"));
    onebjet_loose_plots_[1] = new HttPlots(fs_->mkdir(pt_cat_string+"_loose_ss_sel"));
    onebjet_loose_plots_[2] = new HttPlots(fs_->mkdir(pt_cat_string+"_loose_os_con"));
    onebjet_loose_plots_[3] = new HttPlots(fs_->mkdir(pt_cat_string+"_loose_ss_con"));

    return 0;
  }

  int HttOneBJetCategory::Execute(TreeEvent *event) {
    std::map<std::string, bool> cat_status;
    if (event->Exists("cat_status")) {
      cat_status = event->Get< std::map<std::string, bool> >("cat_status");
    }
    std::string cat_label = high_pt_ ? "onebjet_highpt" : "onebjet_lowpt";
    ...
    if (apply_special_mode_) {
      ...
      if (jets.size() >= special_min_ && jets.size() <= special_max_) {
        if (fs_ && sel_mode < 4) {
          onebjet_yields_[sel_mode] += wt;
          onebjet_plots_[sel_mode]->FillLeptonMetPlots(*(dilepton[0]), *pfMVAMet, wt);
          onebjet_plots_[sel_mode]->FillPt20JetPlots(jets, wt, 1.0);
          if (event->Exists("svfitMass")) onebjet_plots_[sel_mode]->FillSVFitMassPlot(event->Get<double>("svfitMass"),wt);
        }
        if (fs_ && sel_mode%2 == 0) onebjet_plots_[0]->FillIncMT(*(dilepton[0]), *pfMVAMet, wt);
        if (fs_ && sel_mode%2 == 1) onebjet_plots_[1]->FillIncMT(*(dilepton[0]), *pfMVAMet, wt);
        if (fs_ && sel_mode == 2) onebjet_plots_[2]->FillIncMT(*(dilepton[0]), *pfMVAMet, wt);
        if (fs_ && sel_mode == 3) onebjet_plots_[3]->FillIncMT(*(dilepton[0]), *pfMVAMet, wt);
    ...
    if (btag_jets.size() > 0) {
      if (fs_ && sel_mode < 4) {
        onebjet_loose_yields_[sel_mode] += wt;
        std::vector<PFJet*> jets = event->GetPtrVec<PFJet>(jets_label_); // Make a copy of the jet collection
        onebjet_loose_plots_[sel_mode]->FillLeptonMetPlots(*(dilepton[0]), *pfMVAMet, wt);
        onebjet_loose_plots_[sel_mode]->FillJetPlots(jets, wt);
        onebjet_loose_plots_[sel_mode]->FillPt20JetPlots(btag_jets, wt, 1.0);
        if (event->Exists("svfitMass")) onebjet_loose_plots_[sel_mode]->FillSVFitMassPlot(event->Get<double>("svfitMass"),wt);
      }
      if (fs_ && sel_mode%2 == 0) onebjet_loose_plots_[0]->FillIncMT(*(dilepton[0]), *pfMVAMet, wt);
      if (fs_ && sel_mode%2 == 1) onebjet_loose_plots_[1]->FillIncMT(*(dilepton[0]), *pfMVAMet, wt);
      if (fs_ && sel_mode == 2) onebjet_loose_plots_[2]->FillIncMT(*(dilepton[0]), *pfMVAMet, wt);
      if (fs_ && sel_mode == 3) onebjet_loose_plots_[3]->FillIncMT(*(dilepton[0]), *pfMVAMet, wt);
    }
    ...
    if (fs_ && sel_mode < 4) {
      ...
      std::vector<PFJet*> jets = event->GetPtrVec<PFJet>(jets_label_); // Make a copy of the jet collection
      onebjet_plots_[sel_mode]->FillLeptonMetPlots(*(dilepton[0]), *pfMVAMet, wt);
      onebjet_plots_[sel_mode]->FillJetPlots(jets, wt);
      onebjet_plots_[sel_mode]->FillPt20JetPlots(jets, wt, 1.0);
      if (event->Exists("svfitMass")) onebjet_plots_[sel_mode]->FillSVFitMassPlot(event->Get<double>("svfitMass"),wt);
    }
    if (fs_ && sel_mode%2 == 0) onebjet_plots_[0]->FillIncMT(*(dilepton[0]), *pfMVAMet, wt);
  ...
  int HttOneBJetCategory::PostAnalysis() {
    std::cout << "----------------------------------------" << std::endl;
    std::cout << "Post-Analysis Info for HTT OneBJet Category" << std::endl;
    std::cout << "----------------------------------------" << std::endl;
    std::cout << boost::format("%-15s %-10s %-10s %-10s %-10s\n") 
      % "Sel Mode:" % "OS-Sel" % "SS-Sel" % "OS-Con" % "SS-Con";
    std::cout << boost::format("%-15s %-10s %-10s %-10s %-10s\n") 
      % (std::string(high_pt_ ? "onebjet_highpt" : "onebjet_lowpt")+":") % onebjet_yields_[0] % onebjet_yields_[1] % onebjet_yields_[2] % onebjet_yields_[3];
  ...
  void HttOneBJetCategory::PrintInfo() {
    ;
  }
}
