...
namespace ic {
  ...
  float Jet::GetJecFactor(std::string const& name) const {
    ...
    if (iter != jec_factors_.end()) {
      ...
      std::cerr << "Warning in <Jet::GetJecFactor>: JEC Factor \"" 
  ...
  float Jet::GetBDiscriminator(std::string const& name) const {
    ...
    if (iter != b_discriminators_.end()) {
      ...
      std::cerr << "Warning in <Jet::GetBDiscriminator>: Algorithm \"" 
          << name << "\" not found" << std::endl;
      return 0.0;
    }
  }
  ...
  std::string Jet::UnHashJecFactor(std::size_t jec) const{
    ...
    if (it != dict.end()) {
      ...
      std::cerr << "Warning in <Jet>: Unable to unhash \"" <<
          jec << "\", returning empty string" << std::endl;
    }
    return "";
  }
  ...
  std::string Jet::UnHashDiscr(std::size_t dis) const {
    ...
    static bool is_set = false;
    if (!is_set) {
      dict[CityHash64("combinedSecondaryVertexBJetTags")]                     = "combinedSecondaryVertexBJetTags";
      dict[CityHash64("combinedSecondaryVertexMVABJetTags")]                  = "combinedSecondaryVertexMVABJetTags";
      dict[CityHash64("jetBProbabilityBJetTags")]               
        = "jetBProbabilityBJetTags";
      dict[CityHash64("jetProbabilityBJetTags")]                
      ...
      dict[CityHash64("softMuonBJetTags")]                      
        = "softMuonBJetTags";
      dict[CityHash64("softMuonByIP3dBJetTags")]                
        = "softMuonByIP3dBJetTags";
      dict[CityHash64("softMuonByPtBJetTags")]                  
