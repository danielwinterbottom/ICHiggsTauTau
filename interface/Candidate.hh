...
namespace ic {
  ...
  class Candidate {
    ...
    public:
       ...
       * variables can be calculated. Please note, specific getter functions are
       * provided by the Candidate class for the most commonly used
       * variables(pt, eta, phi, energy and mass). */
      inline Vector const& vector() const { return vector_; }
      inline void set_vector(Vector const& vector) { vector_ = vector; }

      /** The object ID */
      /** Each Candidate object (and any object deriving from it) can have a
       * unique ID assigned.  This makes it possible for one object to refer to
       * any number of other objects in a given event.  For example, A GenJet
       * may contain the IDs of its GenParticle constituents. In another case, a
       * PFJet contain the IDs of its constituent tracks. */
