...
namespace ic {
  ...
  class CaloJet : public Jet {
    public:
      ...
      inline float const& max_em_tower_energy() const { return max_em_tower_energy_; }
      inline void set_max_em_tower_energy(float const& max_em_tower_energy) { max_em_tower_energy_ = max_em_tower_energy; }

      inline float const& max_had_tower_energy() const { return max_had_tower_energy_; }
      inline void set_max_had_tower_energy(float const& max_had_tower_energy) { max_had_tower_energy_ = max_had_tower_energy; }

      inline float const& energy_frac_had() const { return energy_frac_had_; }
      inline void set_energy_frac_had(float const& energy_frac_had) { energy_frac_had_ = energy_frac_had; }

      inline float const& energy_frac_em() const { return energy_frac_em_; }
      inline void set_energy_frac_em(float const& energy_frac_em) { energy_frac_em_ = energy_frac_em; }

      inline float const& had_energy_HB() const { return had_energy_HB_; }
      inline void set_had_energy_HB(float const& had_energy_HB) { had_energy_HB_ = had_energy_HB; }

      inline float const& had_energy_HO() const { return had_energy_HO_; }
      inline void set_had_energy_HO(float const& had_energy_HO) { had_energy_HO_ = had_energy_HO; }

      inline float const& had_energy_HE() const { return had_energy_HE_; }
      inline void set_had_energy_HE(float const& had_energy_HE) { had_energy_HE_ = had_energy_HE; }

      inline float const& had_energy_HF() const { return had_energy_HF_; }
      inline void set_had_energy_HF(float const& had_energy_HF) { had_energy_HF_ = had_energy_HF; }

      inline float const& em_energy_EB() const { return em_energy_EB_; }
      inline void set_em_energy_EB(float const& em_energy_EB) { em_energy_EB_ = em_energy_EB; }

      inline float const& em_energy_EE() const { return em_energy_EE_; }
      inline void set_em_energy_EE(float const& em_energy_EE) { em_energy_EE_ = em_energy_EE; }

      inline float const& em_energy_HF() const { return em_energy_HF_; }
      inline void set_em_energy_HF(float const& em_energy_HF) { em_energy_HF_ = em_energy_HF; }

      inline float const& towers_area() const { return towers_area_; }
      inline void set_towers_area(float const& towers_area) { towers_area_ = towers_area; }
